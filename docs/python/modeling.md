...
Use {py:meth}`highspy.Highs.val` to read back the value of a single variable
(or linear expression) after solving, or {py:meth}`highspy.Highs.vals` for a
...
{py:meth}`highspy.Highs.addVar` and {py:meth}`highspy.Highs.addRow`, with
costs updated afterwards via {py:meth}`highspy.Highs.changeColCost`:
...
For full control, populate a {py:class}`highspy.HighsLp` object and pass it
to HiGHS.
