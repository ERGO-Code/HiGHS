...
class HighsSparseMatrix {
 public:
  ...
  double computeDot(const HVector& column,
		    const HighsInt use_col) const {return computeDot(column.array, use_col);}

  double computeDot(const std::vector<double>& array, const HighsInt use_col) const;
