...
void HModel::setup_numBasicLogicals() {
  numBasicLogicals = 0;
  for (int i = 0; i < numRow; i++) if (basicIndex[i] >= numCol) numBasicLogicals += 1;
...
void HModel::util_deleteRowset(int* dstat) {
  ...
  for (int row=0; row < numRow; row++) {
    if (!dstat[row]) {
      ...
      if (rp) printf("   Row %4d: dstat = %2d: Variable %2d becomes %2d; [%11g, %11g]; nonbasicFlag = %2d; nonbasicMove = %2d\n", 
		     row, dstat[row], var, newVar, rowLower[newRow], rowUpper[newRow], nonbasicFlag[newVar], nonbasicMove[newVar]);
      newRow++;
    } else {
      //Row is deleted
      dstat[row] = -1;
      if (rp) printf("   Row %4d: dstat = %2d: Variable %2d is deleted\n", row, dstat[row], numCol+row);
    }
  }
  
  if (rp) {
    printf("After\n");
    for (int row=0; row < numRow; row++) printf("   Row %4d: dstat = %2d\n", row, dstat[row]);
  }
...
#ifdef SCIP_dev
  if (rp) printf("Had %d rows; removed %d rows; now %d rows\n", numRow, dlNumRow, newRow);
#endif
  numRow -= dlNumRow;
  numTot -= dlNumRow;
  ...
  if (rp) {
    ...
    for (int row=0; row < numRow; row++) printf("Basic variable in row %2d is %2d\n", row, basicIndex[row]);
    for (int col=0; col < numCol; col++) printf("Col %2d has nonbasicFlag = %2d\n", col, nonbasicFlag[col]);
    for (int row=0; row < numRow; row++) printf("Row %2d (Variable %2d) has nonbasicFlag = %2d\n", row, numCol+row, nonbasicFlag[numCol+row]);
  }
