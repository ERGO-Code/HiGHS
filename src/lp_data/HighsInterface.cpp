...
HighsStatus Highs::addColsInterface(HighsInt XnumNewCol, const double* XcolCost,
  ...
  if (valid_simplex_lp)
    assert(ekk_instance.lp_.dimensionsOk("addCols - simplex"));
...
HighsStatus Highs::addRowsInterface(HighsInt XnumNewRow,
  ...
  } else if (lp.a_matrix_.isRowwise()) {
    assert(1 == 0);
  ...
  if (valid_simplex_lp)
    assert(ekk_instance.lp_.dimensionsOk("addRows - simplex"));
  return return_status;
}
...
HighsStatus Highs::deleteColsInterface(HighsIndexCollection& index_collection) {
  ...
  if (valid_simplex_lp)
    assert(ekk_instance.lp_.dimensionsOk("deleteCols - simplex"));
...
HighsStatus Highs::deleteRowsInterface(HighsIndexCollection& index_collection) {
  ...
  if (valid_simplex_lp)
    assert(ekk_instance.lp_.dimensionsOk("deleteRows - simplex"));
  return HighsStatus::kOk;
}
...
HighsStatus Highs::getCoefficientInterface(const HighsInt Xrow,
  ...
  for (HighsInt el = model_.lp_.a_matrix_.start_[Xcol];
    ...
    if (model_.lp_.a_matrix_.index_[el] == Xrow) {
      value = model_.lp_.a_matrix_.value_[el];
      break;
    }
  }
  return HighsStatus::kOk;
