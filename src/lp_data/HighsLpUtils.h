...
HighsStatus assessLpDimensions(const HighsOptions& options,
			       const HighsLp& lp);

HighsStatus assessCosts(const HighsOptions& options,
			const int col_ix_os, const int mask_num_col,
...
HighsStatus assessBounds(const HighsOptions& options,
			 const char* type, const int ix_os,
...
void writeSolutionToFile(FILE* file,
			 const HighsLp& lp,
...
HighsStatus assessIntervalSetMask(const HighsOptions& options,
				  const int max_ix, const bool interval,
...
double vectorProduct(const std::vector<double>& v1, const std::vector<double>& v2);
