...
HighsDebugStatus debugHighsSolution(const string message,
  ...
  // warning.
  resetModelStatusAndSolutionParams(dummy_model_status, dummy_highs_info,
                                    options);
...
HighsDebugStatus debugHighsSolution(
  ...
  getKktFailures(lp, gradient, solution, basis, local_highs_info,
  ...
  if (check_model_status_and_highs_info) {
    ...
    return_status = debugCompareSolutionParams(options, highs_info,
                                               local_highs_info);
