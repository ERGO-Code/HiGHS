...
void HighsLp::unapplyMods() {
  ...
  if (!num_lower_bound) assert(!relaxed_semi_variable_lower_value.size());
  ...
  // Restore any tighteneed upper bounds
  ...
  const HighsInt num_upper_bound = tightened_semi_variable_upper_bound_index.size();

  // Ensure that if there are no indices of modified upper bounds,
  // then there are no modified upper bound values
  if (!num_upper_bound) assert(!tightened_semi_variable_upper_bound_value.size());
...
bool HighsLpMods::isClear() {
  ...
  if (this->save_inconsistent_semi_variable_lower_bound_value.size()) return false;
  if (this->save_inconsistent_semi_variable_upper_bound_value.size()) return false;
