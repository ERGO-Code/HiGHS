...
Status loadOptions(int argc, char** argv, Options& options_) {
    ...
    std::cout << "Built with CMAKE_BUILD_TYPE=" << CMAKE_BUILD_TYPE
              << std::endl;
...
Status solveSimplex(options, LP, solution);
  ...
  */
  ...
  int solveSCIP(const char *filename) {
    ...
    model.util_addCols(numExtractCols, &XcolCost[0], &XcolLower[0],
                       &XcolUpper[0], nnonz, &XAstart[0], &XAindex[0],
                       &XAvalue[0]);
    //  model.util_reportModel();
    ...
    for (int col = 0; col < model.numCol; col++) {
      ...
      if (model.nonbasicFlag[col]) {
        ...
        model.util_chgColBoundsSet(1, &colBoundIndex, &nw_colLower,
                                   &nw_colUpper);
        ...
        // avgas
        model.util_chgColBoundsSet(1, &colBoundIndex, &og_colLower,
                                   &og_colUpper);
  ...
  int solvePlainJAJH(const char *Price_ArgV, const char *EdWt_ArgV,
    ...
    if (with_presolve) {
      ...
      if (model.problemStatus != LP_Status_OutOfTime) {
        ...
        // model
        model.util_getPrimalDualValues(pre->colValue, pre->colDual,
                                       pre->rowValue, pre->rowDual);
        model.util_getBasicIndexNonbasicFlag(pre->basicIndex,
                                             pre->nonbasicFlag);
  ...
  Status presolve(const LpData &lp, LpData &reduced_lp) {
    ...
    if (status == HPresolve::Unset) {
      ...
      else if (status == HPresolve::Unbounded) mod.problemStatus =
          LP_Status_Unbounded;
      else {
      ...
      double presolve(HModel & mod, double &time) {
        ...
        if (status == HPresolve::Unset) {
          ...
          solver.solve(&mod);
          pre->setProblemStatus(mod.getPrStatus());
          mod.util_getPrimalDualValues(pre->colValue, pre->colDual,
                                       pre->rowValue, pre->rowDual);
          mod.util_getBasicIndexNonbasicFlag(pre->basicIndex,
                                             pre->nonbasicFlag);
          pre->postsolve();
      ...
      int solvePlainExperiments(const char *filename) {
        ...
        model.util_getPrimalDualValues(colValue2, colDual2, rowValue2,
                                       rowDual2);
      ...
      void copyMatrix(const Problem<double> &problem, vector<int> &Astart,
        ...
        Aendtmp = Aend;
        const int *Aindex_ =
            problem.getConstraintMatrix().getTransposeRowIndices();
        const double *Avalue_ =
            problem.getConstraintMatrix().getTransposeValues();
      ...
      int solveExternalPresolve(const char *fileName) {
        ...
        // set up data
        SparseStorage<double> matrix_transpose(
            &model.Avalue[0], &model.Astart[0], &model.Aindex[0], model.numCol,
        ...
        problem.setConstraintMatrix(matrix_transpose, model.rowLower,
                                    model.rowUpper, true);
        ...
        vector<int> Astart =
            problem.getConstraintMatrix().getTransposeColStart();
        ...
        // if not needed do not make matrix copy
          ...
          Aindex_p =
              (int *)problem.getConstraintMatrix().getTransposeRowIndices();
          Avalue_p =
              (double *)problem.getConstraintMatrix().getTransposeValues();
        }
        ...
        model.load_fromArrays(
            nCols, 1, &(problem.getObjective().coefficients[0]), &colLower[0],
            &colUpper[0], nRows, &rowLower[0], &rowUpper[0],
            problem.getConstraintMatrix().getNnz(), &Astart[0], Aindex_p,
            Avalue_p);
