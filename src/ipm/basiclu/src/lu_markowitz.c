...
lu_int lu_markowitz(struct lu* this) {
  ...
  const lu_int nz_start =
      search_rows ? MIN(this->min_colnz, this->min_rownz) : this->min_colnz;
  ...
  for (nz = nz_start; nz <= m; nz++) {
    ...
    for (j = colcount_flink[m + nz]; j < m; j = colcount_flink[j]) {
      if (min_colnz == -1) min_colnz = nz;
      assert(Wend[j] - Wbegin[j] == nz);
      cmx = colmax[j];
      assert(cmx >= 0);
      if (!cmx || cmx < abstol) continue;
      ...
      for (pos = Wbegin[j]; pos < Wend[j]; pos++) {
        ...
        if (mc < MC) {
          ...
          pivot_row = i;
          pivot_col = j;
          if (search_rows && MC <= (nz1 - 1) * (nz1 - 1)) goto done;
        }
      }
      /* We have seen at least one eligible pivot in column j. */
      assert(MC < M * M);
      if (++nsearch >= maxsearch) goto done;
    }
    assert(j == m + nz);

    if (!search_rows) continue;

    /* Search rows with nz nonzeros. */
    for (i = rowcount_flink[m + nz]; i < m; i = inext) {
      if (min_rownz == -1) min_rownz = nz;
  ...
  if (min_colnz >= 0) this->min_colnz = min_colnz;
  if (min_rownz >= 0) this->min_rownz = min_rownz;
