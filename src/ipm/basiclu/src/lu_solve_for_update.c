...
lu_int lu_solve_for_update(struct lu* this, const lu_int nrhs,
                           ...
                           lu_int* p_nlhs, lu_int* ilhs, double* xlhs,
                           char trans) {
  ...
  lu_int i, j, k, n, t, top, pos, put, ipivot, jpivot, nz, nz_symb, M, room,
      need, jbegin, jend;
  ...
  if (trans == 't' || trans == 'T') {
     ...
     */
    M = ++this->marker;
    top = lu_solve_symbolic(m, Lbegin, NULL, Lindex, nrhs, irhs, pattern_symb,
                            pstack, marked, M);
    nz_symb = m - top;

    for (n = 0; n < nrhs; n++) work[irhs[n]] = xrhs[n];
    nz = lu_solve_triangular(nz_symb, pattern_symb + top, Lbegin, NULL, Lindex,
                             Lvalue, NULL, droptol, work, pattern, &Lflops);
    ...
    if (nz < nz_symb) {
      ...
      for (; t < m; t++) marked[pattern_symb[t]]--;
    }
    ...
    need = nz + 1;
    if (room < need) {
      for (n = 0; n < nz; n++) work[pattern[n]] = 0.0;
    ...
    if (nz <= nz_sparse) {
      ...
      nz =
          lu_solve_triangular(nz_symb, pattern_symb + top, Ubegin, NULL, Uindex,
                              Uvalue, row_pivot, droptol, work, ilhs, &Uflops);
