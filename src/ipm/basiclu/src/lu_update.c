 ...
 *
 */
static lu_int find(const lu_int j, const lu_int* index, lu_int start,
                   const lu_int end) {
  if (end >= 0) {
    while (start < end && index[start] != j) start++;
    return start;
  } else {
    while (index[start] != j && index[start] >= 0) start++;
 ...
 *
 */
static lu_int bfs_path(const lu_int m, /* graph has m nodes */
                       const lu_int j0, const lu_int* begin, const lu_int* end,
                       const lu_int* index, lu_int* jlist, lu_int* marked,
  ...
  for (pos = 0; pos < tail; pos++) marked[queue[pos]] = 0; /* reset */
...
static void check_consistency(struct lu* this, lu_int* p_col, lu_int* p_row) {
  ...
  for (i = 0; i < m; i++) /* process column file entries */
  {
    ...
    for (pos = Ubegin[i]; (ientry = Uindex[pos]) >= 0; pos++) {
      jentry = qmap[ientry];
      for (where = Wbegin[jentry]; where < Wend[jentry] && Windex[where] != j;
           where++)
        ;
  ...
  for (j = 0; j < m; j++) /* process row file entries */
  {
    ...
    for (pos = Wbegin[j]; pos < Wend[j]; pos++) {
      jentry = Windex[pos];
      ientry = pmap[jentry];
      for (where = Ubegin[ientry]; Uindex[where] >= 0 && Uindex[where] != i;
           where++)
        ;
...
lu_int lu_update(struct lu* this, double xtbl) {
  ...
  for (pos = Ubegin[ipivot]; (i = Uindex[pos]) >= 0; pos++) {
    ...
    if (Wend[j] == Wbegin[jnext]) {
      ...
      lu_file_reappend(j, m, Wbegin, Wend, Wflink, Wblink, Windex, Wvalue,
                       room);
    }
    end = Wend[j]++;
    Windex[end] = jpivot;
  ...
  if (have_diag) {
    ...
    for (t = top; t < m - 1 && istriangular; t++) {
      ...
      rtop = lu_dfs(j, Wbegin, Wend, Windex, rtop, reach, pstack, marked, M);
      assert(reach[rtop] == j);
      reach[rtop] = jnext;
  ...
  if ((used - need) > this->compress_thres * used) {
    nz =
        lu_file_compress(m, Wbegin, Wend, Wflink, Windex, Wvalue, stretch, pad);
    assert(nz == Unz);
  }
