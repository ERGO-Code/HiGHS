 ...
 */
#ifndef NDEBUG
  for (k = 0; k < m; k++) pivotrow[k] = -1;
  for (k = 0; k < m; k++) pivotcol[k] = -1;
#endif

  lrank = rank;
  for (i = 0; i < m; i++) {
    if (pinv[i] < 0) pinv[i] = lrank++;
    pivotrow[pinv[i]] = i;
  }
  assert(lrank == m);
  lrank = rank;
  for (j = 0; j < m; j++) {
    if (qinv[j] < 0) qinv[j] = lrank++;
    pivotcol[qinv[j]] = j;
  }
  assert(lrank == m);

#ifndef NDEBUG
  for (k = 0; k < m; k++) assert(pivotrow[k] >= 0);
  for (k = 0; k < m; k++) assert(pivotcol[k] >= 0);
#endif

  /* Dependent columns get unit pivot elements. */
  for (k = rank; k < m; k++) col_pivot[pivotcol[k]] = 1.0;
  ...
  for (i = 0; i < m; i++) Lbegin[i] = Lbegin_p[pinv[i]];
...
#ifndef NDEBUG
  for (i = 0; i < m; i++) assert(Lindex[iwork1[i]] == -1);
#endif
...
#ifndef NDEBUG
  for (j = 0; j < m; j++) assert(Uindex[iwork1[j]] == -1);
#endif
