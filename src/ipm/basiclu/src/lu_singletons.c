...
static lu_int singleton_cols(const lu_int m,
                             const lu_int* Bbegin, /* B columnwise */
                             const lu_int* Bend, const lu_int* Bi,
                             const double* Bx,
                             const lu_int* Btp, /* B rowwise */
                             const lu_int* Bti, const double* Btx, lu_int* Up,
                             lu_int* Ui, double* Ux, lu_int* Lp, lu_int* Li,
                             double* Lx, double* col_pivot, lu_int* pinv,
                             lu_int* qinv, lu_int* iset, /* size m workspace */
                             lu_int* queue,              /* size m workspace */
                             lu_int rank, double abstol);

static lu_int singleton_rows(const lu_int m,
                             const lu_int* Bbegin, /* B columnwise */
                             const lu_int* Bend, const lu_int* Bi,
                             const double* Bx,
                             const lu_int* Btp, /* B rowwise */
...
lu_int lu_singletons(struct lu* this, const lu_int* Bbegin, const lu_int* Bend,
  ...
  if (!ok) return BASICLU_ERROR_invalid_argument;
  ...
  if (!ok) return BASICLU_REALLOCATE;
  ...
  for (i = 0; i < m; i++) pinv[i] = -1;
  for (j = 0; j < m; j++) qinv[j] = -1;

  if (nzbias >= 0) /* put more in U */
  {
    Lbegin_p[0] = Ubegin[0] = rank = 0;

    rank = singleton_cols(m, Bbegin, Bend, Bi, Bx, Btp, Bti, Btx, Ubegin,
                          Uindex, Uvalue, Lbegin_p, Lindex, Lvalue, col_pivot,
                          pinv, qinv, iwork1, iwork2, rank, abstol);

    rank = singleton_rows(m, Bbegin, Bend, Bi, Bx, Btp, Bti, Btx, Ubegin,
                          Uindex, Uvalue, Lbegin_p, Lindex, Lvalue, col_pivot,
                          pinv, qinv, iwork1, iwork2, rank, abstol);
  } else /* put more in L */
  {
  ...
  for (i = 0; i < m; i++)
    if (pinv[i] < 0) pinv[i] = -1;
  for (j = 0; j < m; j++)
    if (qinv[j] < 0) qinv[j] = -1;
...
static lu_int singleton_cols(const lu_int m,
  ...
  for (front = 0; front < tail; front++) {
    ...
    if (qinv[j] == -1) continue; /* empty column in active submatrix */
    i = iset[j];
    assert(i >= 0 && i < m);
    assert(pinv[i] < 0);
    end = Btp[i + 1];
    for (pos = Btp[i]; Bti[pos] != j; pos++) /* find pivot */
      assert(pos < end - 1);
    piv = Btx[pos];
    if (!piv || fabs(piv) < abstol) continue; /* skip singularity */
    ...
    for (pos = Btp[i]; pos < end; pos++) {
         ...
         not be the entire matrix (rows eliminated before) */
      {
        ...
        if (++qinv[j2] == -2) queue[tail++] = j2; /* new singleton */
      }
    }
    Up[rank + 1] = put;
    col_pivot[j] = piv;
...
static lu_int singleton_rows(const lu_int m,
  ...
  for (front = 0; front < tail; front++) {
    ...
    if (pinv[i] == -1) continue; /* empty column in active submatrix */
    j = iset[i];
    assert(j >= 0 && j < m);
    assert(qinv[j] < 0);
    end = Bend[j];
    ...
    for (pos = Bbegin[j]; pos < end; pos++) {
         ...
         not be the entire matrix (columns eliminated before) */
      {
        ...
        if (++pinv[i2] == -2) queue[tail++] = i2; /* new singleton */
      }
    }
