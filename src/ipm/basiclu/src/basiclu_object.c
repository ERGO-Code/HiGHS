...
static void lu_clear_lhs(struct basiclu_object* obj) {
  ...
  if (nz) {
    if (nz <= nzsparse)
      for (p = 0; p < nz; p++) obj->lhs[obj->ilhs[p]] = 0;
    else
...
lu_int basiclu_obj_initialize(struct basiclu_object* obj, lu_int m) {
  ...
  if (!obj) return BASICLU_ERROR_argument_missing;
  if (m < 0) return BASICLU_ERROR_invalid_argument;
...
lu_int basiclu_obj_factorize(struct basiclu_object* obj, const lu_int* Bbegin,
  ...
  if (!isvalid(obj)) return BASICLU_ERROR_invalid_object;

  status =
      basiclu_factorize(obj->istore, obj->xstore, obj->Li, obj->Lx, obj->Ui,
                        obj->Ux, obj->Wi, obj->Wx, Bbegin, Bend, Bi, Bx, 0);

  while (status == BASICLU_REALLOCATE) {
    status = lu_realloc_obj(obj);
    if (status != BASICLU_OK) break;
    status =
        basiclu_factorize(obj->istore, obj->xstore, obj->Li, obj->Lx, obj->Ui,
                          obj->Ux, obj->Wi, obj->Wx, Bbegin, Bend, Bi, Bx, 1);
  }
...
lu_int basiclu_obj_solve_for_update(struct basiclu_object* obj, lu_int nzrhs,
  ...
  while (status == BASICLU_OK) {
    status = basiclu_solve_for_update(
        ...
        obj->Wx, nzrhs, irhs, xrhs, want_solution ? &obj->nzlhs : NULL,
...
lu_int basiclu_obj_update(struct basiclu_object* obj, double xtbl) {
  ...
  while (status == BASICLU_OK) {
    status = basiclu_update(obj->istore, obj->xstore, obj->Li, obj->Lx, obj->Ui,
                            obj->Ux, obj->Wi, obj->Wx, xtbl);
    if (status != BASICLU_REALLOCATE) break;
    status = lu_realloc_obj(obj);
  }
