...
lu_int lu_pivot(struct lu* this) {
  ...
  if (status == BASICLU_OK) {
    for (pos = Ubegin[rank]; pos < Ubegin[rank + 1]; pos++) {
      j = Uindex[pos];
      assert(j != pivot_col);
      if (colmax[j] == 0.0 || colmax[j] < this->abstol) lu_remove_col(this, j);
    }
  }
...
static lu_int lu_pivot_any(struct lu* this) {
  ...
  for (rpos = rbeg + 1; rpos < rend; rpos++) {
    ...
    if (room < cnz1) {
      ...
      lu_file_reappend(j, 2 * m, Wbegin, Wend, Wflink, Wblink, Windex, Wvalue,
                       room);
      put = Wend[j];
    ...
    a = xrj / pivot;
    for (pos = 1; pos <= cnz1; pos++) work[pos] -= a * wx[pos];
    for (pos = 1; pos <= cnz1; pos++) {
      Windex[put] = wi[pos];
      Wvalue[put++] = work[pos];
      if ((x = fabs(work[pos])) > cmx) cmx = x;
      work[pos] = 0.0;
    }
    Wend[j] = put;
  ...
  for (pos = cbeg + 1; pos < cend; pos++) marked[Windex[pos]] = 0;

  /* ---------------------------------------------------------------------- */
  /* Row file update */
  /* ---------------------------------------------------------------------- */

  for (rpos = rbeg; rpos < rend; rpos++) marked[Windex[rpos]] = 1;
  ...
  for (pos = cbeg + 1; pos < cend; pos++) {
    ...
    for (rpos = Wbegin[m + i]; rpos < Wend[m + i]; rpos++) {
      if ((j = Windex[rpos]) == pivot_col) found = 1;
      if (!marked[j]) Windex[put++] = j;
    }
    assert(found);
    Wend[m + i] = put;
    ...
    for (rpos = rbeg + 1; rpos < rend; rpos++) Windex[put++] = Windex[rpos];
    Wend[m + i] = put;
  ...
  for (rpos = rbeg; rpos < rend; rpos++) marked[Windex[rpos]] = 0;
 ...
 */
#ifdef DEBUG_EXTRA
  assert(lu_file_diff(m, Wbegin + m, Wend + m, Wbegin, Wend, Windex, NULL) ==
         0);
  assert(lu_file_diff(m, Wbegin, Wend, Wbegin + m, Wend + m, Windex, NULL) ==
         0);
#endif
...
static lu_int lu_pivot_doubleton_col(struct lu* this) {
  ...
  for (rpos = rbeg + 1; rpos < rend; rpos++) {
    ...
    if (where_other == -1) {
      ...
      Windex[where_pivot] = Windex[end];
      Wvalue[where_pivot] = Wvalue[end];
      if (where_other == end) where_other = where_pivot;
  ...
  if (nfill > room) {
    ...
    lu_file_reappend(m + other_row, 2 * m, Wbegin, Wend, Wflink, Wblink, Windex,
                     Wvalue, space);
    this->nexpand++;
  }
  ...
  for (pos = rbeg + 1; pos < rend; pos++) Windex[put++] = Windex[pos];
  Wend[m + other_row] = put;
