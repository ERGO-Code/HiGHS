...
lu_int basiclu_get_factors(lu_int istore[], double xstore[], lu_int Li[],
                           ...
                           lu_int Lcolptr[], lu_int Lrowidx[], double Lvalue_[],
  ...
  if (rowperm) memcpy(rowperm, this.pivotrow, m * sizeof(lu_int));
  if (colperm) memcpy(colperm, this.pivotcol, m * sizeof(lu_int));
  ...
  if (Ucolptr && Urowidx && Uvalue_) {
    ...
    for (j = 0; j < m; j++) {
      for (pos = Wbegin[j]; pos < Wend[j]; pos++) colptr[Windex[pos]]++;
    }
    put = 0;
