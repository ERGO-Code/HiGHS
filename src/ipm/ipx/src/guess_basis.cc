...
static Int ComputePattern(const SparseMatrix& L, const SparseMatrix& AI, Int j,
                          const Int* rownumber, Int* pattern, Int* pstack,
                          Int* marked, Int marker) {
  const Int m = L.rows();
  ...
  for (Int p = AI.begin(j); p < AI.end(j); p++) {
    Int i = AI.index(p);
    if (marked[i] != marker)
      top = DepthFirstSearch(i, Lp, Li, rownumber, top, pattern, marked, marker,
                             pstack);
  }
  return top;
}
...
static Int ComputeValues(const SparseMatrix& L, const SparseMatrix& AI, Int j,
                         const Int* rownumber, const Int* pattern, Int top,
                         Vector& lhs) {
  const Int m = L.rows();
  const Int* Lp = L.colptr();
  const Int* Li = L.rowidx();
  const double* Lx = L.values();
  for (Int t = top; t < m; t++) lhs[pattern[t]] = 0.0;
  ...
  for (Int t = top; t < m; t++) {
    ...
    if (temp != 0.0) {
      if (k >= 0) {
        ...
        for (Int p = Lp[k]; p < Lp[k + 1]; p++) lhs[Li[p]] -= Lx[p] * temp;
...
static void ProcessFreeColumns(const Control& control, const Model& model,
  ...
  Int num_free = 0;
  for (Int j = 0; j < n + m; j++) {
    if (weights[j] != INFINITY) continue;
    Int top = ComputePattern(L, AI, j, rownumber, pattern.data(), work.data(),
                             marked.data(), j);
    ...
    if (std::abs(pivot) > kLuDependencyTol) {
      ...
      for (Int t = AI.begin(j); t < AI.end(j); t++) {
        Int i = AI.index(t);
        if (rownumber[i] < 0 && lhs[i] != 0.0) L.push_back(i, lhs[i] / pivot);
      }
      L.add_column();
      num_free++;
    }
  ...
  control.Debug() << Textline("Number of free variables in starting basis:")
                  << num_free << '\n';
}
...
static void ProcessSingletons(const Control& control, const Model& model,
  ...
  control.Debug() << Textline("Number of singletons in starting basis:")
                  << num_singletons << '\n';
}
...
static void ProcessRemaining(const Control& control, const Model& model,
  ...
  for (Int j : colperm) {
    if (!active[j]) continue;
    if (weights[j] == 0.0) break;
    bool matched =
        AugmentingPath(j, Ap, Ai, jmatch.data(), cheap.data(), marked.data(),
                       work.data(), work2.data(), work3.data());
    if (matched) {
      basis->push_back(j);
      num_matched++;
    } else {
    ...
    if (num_failed >= 10 * (m - (Int)basis->size())) break;
  }
  for (Int i = 0; i < m; i++) {
    if (jmatch[i] >= 0) {
  ...
  control.Debug() << Textline("Number of other columns matched:") << num_matched
                  << '\n'
                  << Textline("Number of other columns failed:") << num_failed
                  << '\n';
}
...
std::vector<Int> GuessBasis(const Control& control, const Model& model,
  ...
  for (Int i = 0; i < m; i++) {
    if (rownumber[i] < 0) basis.push_back(n + i);
  }
