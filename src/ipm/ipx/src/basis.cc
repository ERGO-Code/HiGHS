...
void Basis::FixNonbasicVariable(Int j) {
  if (StatusOf(j) == NONBASIC_FIXED) return;
...
void Basis::FreeBasicVariable(Int j) {
  const Int m = model_.rows();
  if (StatusOf(j) == BASIC_FREE) return;
...
void Basis::UnfixVariables() {
  ...
  const Int n = model_.cols();
  for (Int j = 0; j < n + m; j++)
    if (map2basis_[j] == -2) map2basis_[j] = -1;
}
...
void Basis::UnfreeVariables() {
  ...
  const Int n = model_.cols();
  for (Int j = 0; j < n + m; j++)
    if (map2basis_[j] >= m) map2basis_[j] -= m;
}

void Basis::SetToSlackBasis() {
  const Int m = model_.rows();
  const Int n = model_.cols();
  for (Int i = 0; i < m; i++) basis_[i] = n + i;
  for (Int j = 0; j < n; j++) map2basis_[j] = -1;
  for (Int i = 0; i < m; i++) map2basis_[n + i] = i;
  Int err = Factorize();
...
Int Basis::Load(const int* basic_status) {
  ...
  if (p != m) return IPX_ERROR_invalid_basis;
...
Int Basis::Factorize() {
  ...
  while (true) {
    ...
    if (flag & 1) {
      control_.Debug(3) << " LU factorization unstable with pivot tolerance "
                        << lu_->pivottol() << '\n';
      // ignore instability
    }
    break;
...
bool Basis::FactorizationIsFresh() const { return factorization_is_fresh_; }
...
void Basis::SolveForUpdate(Int j, IndexedVector& lhs) {
  ...
  if (p < 0) {  // ftran
    ...
    num_ftran_++;
    if (lhs.sparse()) num_ftran_sparse_++;
    time_ftran_ += timer.Elapsed();
  } else {  // btran
    lu_->BtranForUpdate(p, lhs);
    num_btran_++;
    if (lhs.sparse()) num_btran_sparse_++;
...
void Basis::TableauRow(Int jb, IndexedVector& btran, IndexedVector& row,
  ...
  if (is_sparse) {
    ...
    if (nz > kHypersparseThreshold * n) is_sparse = false;
  }
  ...
  if (is_sparse) {
    ...
    for (Int j = 0; j < n + m; j++) {
      ...
      if (map2basis_[j] == -1 || (map2basis_[j] == -2 && !ignore_fixed)) {
        Int begin = AI.begin(j);
        Int end = AI.end(j);
        for (Int p = begin; p < end; p++) result += Ax[p] * btran[Ai[p]];
      }
      row[j] = result;
    }
...
Int Basis::ExchangeIfStable(Int jb, Int jn, double tableau_entry, int sys,
  ...
  *exchanged = false;
  ...
  if (err != 0) {
    ...
    control_.Debug(3) << " stability check forced refactorization after "
  ...
  *exchanged = true;
  ...
  if (lu_->NeedFreshFactorization()) return Factorize();
  return 0;
}
...
void Basis::ComputeBasicSolution(Vector& x, Vector& y, Vector& z) const {
  ...
  y = b;
  for (Int j = 0; j < n + m; j++)
    if (IsNonbasic(j)) ScatterColumn(AI, j, -x[j], y);
  SolveDense(y, y, 'N');
  for (Int p = 0; p < m; p++) x[basis_[p]] = y[p];

  // Compute y and z[nonbasic] so that AI'y+z=c.
  for (Int p = 0; p < m; p++) y[p] = c[basis_[p]] - z[basis_[p]];
  SolveDense(y, y, 'T');
  for (Int j = 0; j < n + m; j++) {
    if (IsNonbasic(j)) z[j] = c[j] - DotColumn(AI, j, y);
  }
}
...
void Basis::ConstructBasisFromWeights(const double* colscale, Info* info) {
  ...
  if (control_.crash_basis()) {
    ...
    control_.Debug() << Textline("Minimum singular value of crash basis:")
                     << sci2(sigma) << '\n';
    Repair(info);
    if (info->basis_repairs < 0) {
      ...
      control_.Debug() << Textline(
                              "Minimum singular value of repaired crash basis:")
                       << sci2(sigma) << '\n';
    }
  } else {
    SetToSlackBasis();
...
double Basis::DensityInverse() const {
  ...
  double density = 0.0;
  for (Int i = 0; i < m; i++) density += 1.0 * rowcounts[i] / m;
  return density / m;
}

const Model& Basis::model() const { return model_; }

Int Basis::factorizations() const { return num_factorizations_; }

Int Basis::updates_total() const { return num_updates_; }
...
double Basis::time_factorize() const { return time_factorize_; }

double Basis::time_ftran() const { return time_ftran_; }

double Basis::time_btran() const { return time_btran_; }

double Basis::time_update() const { return time_update_; }

double Basis::mean_fill() const {
  if (fill_factors_.empty()) return 0.0;
  double mean = 1.0;
  Int num_factors = fill_factors_.size();
  for (double f : fill_factors_) mean *= std::pow(f, 1.0 / num_factors);
  return mean;
}
...
Int Basis::AdaptToSingularFactorization() {
  ...
  for (Int k : dependent_cols) {
    ...
    if (jb >= 0) map2basis_[jb] = -1;  // now NONBASIC
  }
...
bool Basis::TightenLuPivotTol() {
  ...
  if (tol <= 0.05)
    ...
    return false;
  control_.Log() << " LU pivot tolerance tightened to " << lu_->pivottol()
                 << '\n';
  return true;
}
...
void Basis::CrashBasis(const double* colweights) {
  ...
  control_.Debug() << Textline("Number of columns dropped from guessed basis:")
                   << num_dropped << '\n';
  (void)(m);
}
...
static std::tuple<Int, Int, double> InverseSearch(const Basis& basis,
  ...
  for (Int i = 0; i < m; i++) work[i] = 1.0 / (i + 1);

  while (true) {
    basis.SolveDense(work, work, 'N');
    if (!AllFinite(work)) break;
...
void Basis::Repair(Info* info) {
  ...
  while (true) {
    ...
    if (std::abs(pivot) < kBasisRepairThreshold) break;
    Int jb = basis_[pmax];
    Int jn = n + imax;
    if (!IsNonbasic(jn)) {
    ...
    info->basis_repairs++;
    control_.Debug(3) << " basis repair: |pivot| = " << sci2(std::abs(pivot))
                      << '\n';
  }
}
...
void Basis::CrashFactorize(Int* num_dropped) {
  ...
  Int flag =
      lu_->Factorize(begin.data(), end.data(), AI.rowidx(), AI.values(), true);
  num_factorizations_++;
  fill_factors_.push_back(lu_->fill_factor());
  Int ndropped = 0;
  if (flag & 2) ndropped = AdaptToSingularFactorization();
  if (num_dropped) *num_dropped = ndropped;
  ...
  for (Int i = 0; i < m; i++) assert(basis_[i] >= 0);
#endif
}
...
void Basis::PivotFreeVariablesIntoBasis(const double* colweights, Info* info) {
  ...
  for (Int j = 0; j < n + m; j++) {
    if (std::isinf(colweights[j]) && map2basis_[j] < 0) remaining.push_back(j);
  }
  ...
  while (!remaining.empty()) {
    ...
    assert(map2basis_[jn] < 0);
    if ((info->errflag = control_.InterruptCheck()) != 0) return;
    ...
    if (fmax > 4.0 && fmax_nonfree < 1.0) {
      ...
      bool exchanged;
      info->errflag = ExchangeIfStable(jb, jn, ftran[pmax], -1, &exchanged);
      if (info->errflag) return;
      ...
      if (fmax_nonfree <= dependency_tol) {
        ...
        if (!info->cols_inconsistent) {
          ...
          auto update_delta_obj = [&](Int p, double f) {
            Int j = basis_[p];
            if (std::isinf(colweights[j])) delta_obj -= c[j] * f;
          };
        ...
        bool exchanged;
        info->errflag =
            ExchangeIfStable(jb, jn, ftran[pmax_nonfree], -1, &exchanged);
        if (info->errflag) return;
        if (!exchanged)  // factorization was unstable, try again
          continue;
        remaining.pop_back();
    ...
    control_.IntervalLog() << " " << remaining.size()
                           << " free variables remaining\n";
  }
  control_.Debug() << Textline(
                          "Number of free variables swapped for stability:")
                   << stability_pivots << '\n';
}

void Basis::PivotFixedVariablesOutOfBasis(const double* colweights,
                                          Info* info) {
  const Int m = model_.rows();
  const Int n = model_.cols();
  ...
  for (Int j = 0; j < n; j++) {
    ...
    if (colweights[j] == 0.0) assert(map2basis_[j] < 0);
  }
  for (Int j = n; j < n + m; j++) {
    if (colweights[j] == 0.0 && map2basis_[j] >= 0) remaining.push_back(j);
  }
  ...
  while (!remaining.empty()) {
    ...
    if (rmax > 4.0 && rmax_nonfixed < 1.0) {
      ...
      bool exchanged;
      info->errflag = ExchangeIfStable(jb, jmax, row[jmax], 1, &exchanged);
      if (info->errflag) return;
      if (!exchanged)  // factorization was unstable, try again
        continue;
      remaining.pop_back();
      ...
      if (rmax_nonfixed <= dependency_tol) {
        ...
        info->errflag = ExchangeIfStable(jb, jmax_scaled, pivot, 1, &exchanged);
        if (info->errflag) return;
        if (!exchanged)  // factorization was unstable, try again
          continue;
        remaining.pop_back();
  ...
  control_.Debug() << Textline(
                          "Number of fixed variables swapped for stability:")
                   << stability_pivots << '\n';
}
...
Vector CopyBasic(const Vector& x, const Basis& basis) {
  ...
  Vector xbasic(m);
  for (Int p = 0; p < m; p++) xbasic[p] = x[basis[p]];
  return xbasic;
}
