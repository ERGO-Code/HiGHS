...
SparseMatrix::SparseMatrix() { resize(0, 0); }

SparseMatrix::SparseMatrix(Int nrow, Int ncol) { resize(nrow, ncol); }
...
bool SparseMatrix::IsSorted() const {
  for (Int j = 0; j < cols(); j++) {
    for (Int p = begin(j); p < end(j) - 1; p++)
      if (index(p) > index(p + 1)) return false;
  }
  return true;
}
...
void Transpose(const SparseMatrix& A, SparseMatrix& AT) {
  ...
  for (Int p = 0; p < nz; p++) work[A.index(p)]++;
...
void PermuteRows(SparseMatrix& A, const std::vector<Int>& perm) {
  for (Int p = 0; p < A.entries(); p++) A.index(p) = perm[A.index(p)];
}
...
Int RemoveDiagonal(SparseMatrix& A, double* diag) {
  ...
  Int put = 0;
  for (Int j = 0; j < ncol; j++) {
    if (diag) diag[j] = 0.0;  // if no diagonal entry in column j
    Ap[j] = put;
    for (; get < Ap[j + 1]; get++) {
      if (Ai[get] == j) {
        if (diag) diag[j] = Ax[get];
      } else {
        Ai[put] = Ai[get];
        Ax[put] = Ax[get];
        put++;
...
void MultiplyAdd(const SparseMatrix& A, const Vector& rhs, double alpha,
  ...
  if (trans == 't' || trans == 'T') {
    ...
    for (Int j = 0; j < n; j++) lhs[j] += alpha * DotColumn(A, j, rhs);
  } else {
    assert((int)rhs.size() == n);
    assert((int)lhs.size() == m);
    for (Int j = 0; j < n; j++) ScatterColumn(A, j, alpha * rhs[j], lhs);
  }
  (void)(m);
}
  ...
  if (trans == 't' || trans == 'T') {
    if (*uplo == 'u' || *uplo == 'U') {
      ...
      for (Int i = 0; i < ncol; i++) {
        ...
        double d = 0.0;
        for (Int p = begin; p < end; p++) d += x[Ai[p]] * Ax[p];
        x[i] -= d;
        if (!unitdiag) {
          assert(Ai[end] == i);
          x[i] /= Ax[end];
    ...
    if (*uplo == 'u' || *uplo == 'U') {
      ...
      for (Int j = ncol - 1; j >= 0; j--) {
        ...
        double temp = x[j];
        if (temp != 0.0) {
          for (Int p = begin; p < end; p++) x[Ai[p]] -= Ax[p] * temp;
          nz++;
        }
      }
    } else {
...
double Onenorm(const SparseMatrix& A) {
  ...
  for (Int j = 0; j < A.cols(); j++) {
    double colsum = 0.0;
    for (Int p = A.begin(j); p < A.end(j); p++) colsum += std::abs(A.value(p));
...
double NormestInverse(const SparseMatrix& A, const char* uplo, int unitdiag) {
  ...
  if (*uplo == 'u' || *uplo == 'U') {
    for (Int j = 0; j < m; j++) {
      ...
      if (!unitdiag) end--;
      double temp = 0.0;
      for (Int p = begin; p < end; p++) temp -= x[A.index(p)] * A.value(p);
