...
BasicLu::BasicLu(const Control& control, Int dim) : control_(control) {
  ...
  if (status != BASICLU_OK) throw std::logic_error("basiclu_initialize failed");
...
Int BasicLu::_Factorize(const Int* Bbegin, const Int* Bend, const Int* Bi,
  ...
  for (Int ncall = 0;; ncall++) {
    status = basiclu_factorize(istore_.data(), xstore_.data(), Li_.data(),
                               Lx_.data(), Ui_.data(), Ux_.data(), Wi_.data(),
                               Wx_.data(), Bbegin, Bend, Bi, Bx, ncall);
    if (status != BASICLU_REALLOCATE) break;
    Reallocate();
  }
  ...
  control_.Debug(3) << " normLinv = " << sci2(normLinv) << ','
                    << " normUinv = " << sci2(normUinv) << ','
                    << " stability = " << sci2(stability) << '\n';

  Int ret = 0;
  if (stability > kLuStabilityThreshold) ret |= 1;
  if (status == BASICLU_WARNING_singular_matrix) ret |= 2;
  return ret;
}
...
void BasicLu::_GetFactors(SparseMatrix* L, SparseMatrix* U, Int* rowperm,
  ...
  Int status = basiclu_get_factors(
      istore_.data(), xstore_.data(), Li_.data(), Lx_.data(), Ui_.data(),
      Ux_.data(), Wi_.data(), Wx_.data(), rowperm, colperm, Lbegin, Lindex,
      Lvalue, Ubegin, Uindex, Uvalue);
  ...
  if (dependent_cols) {
    ...
    for (Int k = rank; k < dim; k++) dependent_cols->push_back(k);
  }
}

void BasicLu::_SolveDense(const Vector& rhs, Vector& lhs, char trans) {
  Int status = basiclu_solve_dense(
      istore_.data(), xstore_.data(), Li_.data(), Lx_.data(), Ui_.data(),
      Ux_.data(), Wi_.data(), Wx_.data(), &rhs[0], &lhs[0], trans);
  if (status != BASICLU_OK)
...
void BasicLu::_FtranForUpdate(Int nzrhs, const Int* bi, const double* bx) {
  ...
  for (Int ncall = 0;; ncall++) {
    status = basiclu_solve_for_update(istore_.data(), xstore_.data(),
                                      Li_.data(), Lx_.data(), Ui_.data(),
                                      Ux_.data(), Wi_.data(), Wx_.data(), nzrhs,
                                      bi, bx, nullptr, nullptr, nullptr, 'N');
    if (status != BASICLU_REALLOCATE) break;
    Reallocate();
  }
  if (status != BASICLU_OK)
...
void BasicLu::_FtranForUpdate(Int nzrhs, const Int* bi, const double* bx,
  ...
  for (Int ncall = 0;; ncall++) {
    status = basiclu_solve_for_update(
        istore_.data(), xstore_.data(), Li_.data(), Lx_.data(), Ui_.data(),
        Ux_.data(), Wi_.data(), Wx_.data(), nzrhs, bi, bx, &nzlhs,
        lhs.pattern(), lhs.elements(), 'N');
    if (status != BASICLU_REALLOCATE) break;
    Reallocate();
  }
  if (status != BASICLU_OK)
    throw std::logic_error("basiclu_solve_for_update (ftran with lhs) failed");
  lhs.set_nnz(nzlhs);
}
...
void BasicLu::_BtranForUpdate(Int j) {
  ...
  for (Int ncall = 0;; ncall++) {
    status = basiclu_solve_for_update(istore_.data(), xstore_.data(),
                                      Li_.data(), Lx_.data(), Ui_.data(),
                                      Ux_.data(), Wi_.data(), Wx_.data(), 0, &j,
...
void BasicLu::_BtranForUpdate(Int j, IndexedVector& lhs) {
  ...
  for (Int ncall = 0;; ncall++) {
    status = basiclu_solve_for_update(
        istore_.data(), xstore_.data(), Li_.data(), Lx_.data(), Ui_.data(),
        Ux_.data(), Wi_.data(), Wx_.data(), 0, &j, nullptr, &nzlhs,
        lhs.pattern(), lhs.elements(), 'T');
    if (status != BASICLU_REALLOCATE) break;
    Reallocate();
  }
  if (status != BASICLU_OK)
    throw std::logic_error("basiclu_solve_for_update (btran with lhs) failed");
  lhs.set_nnz(nzlhs);
}
...
Int BasicLu::_Update(double pivot) {
  ...
  for (Int ncall = 0;; ncall++) {
    status =
        basiclu_update(istore_.data(), xstore_.data(), Li_.data(), Lx_.data(),
                       Ui_.data(), Ux_.data(), Wi_.data(), Wx_.data(), pivot);
    if (status != BASICLU_REALLOCATE) break;
    Reallocate();
  }
  if (status != BASICLU_OK && status != BASICLU_ERROR_singular_update)
    throw std::logic_error("basiclu_update failed");
  if (status == BASICLU_ERROR_singular_update) return -1;
  ...
  if (pivot_error > kFtDiagErrorTol) {
    control_.Debug(3) << " relative error in new diagonal entry of U = "
                      << sci2(pivot_error) << '\n';
    return 1;
  }
  return 0;
...
double BasicLu::_fill_factor() const { return fill_factor_; }
