...
Int ForrestTomlin::_Factorize(const Int* Bbegin, const Int* Bend, const Int* Bi,
  ...
  // Compute fill factor.
  Int bnz = 0;
  for (Int j = 0; j < dim_; j++) bnz += Bend[j] - Bbegin[j];
  ...
  Int ret = 0;
  if (lu_->stability() > kLuStabilityThreshold) ret |= 1;
  if (!dependent_cols_.empty()) ret |= 2;
  return ret;
}
...
void ForrestTomlin::_GetFactors(SparseMatrix* L, SparseMatrix* U, Int* rowperm,
  ...
  if (L) *L = L_;
  if (U) *U = U_;
  if (rowperm) std::copy(rowperm_.begin(), rowperm_.end(), rowperm);
  if (colperm) std::copy(colperm_.begin(), colperm_.end(), colperm);
  if (dependent_cols) *dependent_cols = dependent_cols_;
}
...
void ForrestTomlin::_FtranForUpdate(Int nb, const Int* bi, const double* bx,
  ...
  for (Int p = 0; p < dim_; p++) lhs[colperm_[p]] = work_[p];
  lhs.InvalidatePattern();
}

void ForrestTomlin::_BtranForUpdate(Int j) { ComputeEta(j); }
...
void ForrestTomlin::_BtranForUpdate(Int j, IndexedVector& lhs) {
  ...
  // Return lhs without pattern.
  for (Int p = 0; p < dim_; p++) lhs[rowperm_[p]] = work_[p];
  lhs.InvalidatePattern();
}
...
Int ForrestTomlin::_Update(double pivot) {
  ...
  Int where = 0;
  while (where < qend && U_.qindex(where) != replace_next_) where++;
  ...
  double newdiag2 =
      (where == qend ? 0.0 : U_.qvalue(where)) - SparseDot(U_, R_);
  ...
  for (Int l = U_.begin(replace_next_); l < end - 1; l++) U_.value(l) = 0.0;
  U_.value(end - 1) = 1.0;
...
bool ForrestTomlin::_NeedFreshFactorization() {
  ...
  if (num_updates == kMaxUpdates) return true;
  if (num_updates < 100) return false;
  if (Rnz > 1.0 * Lnz) {
    // control_.Debug()
    //     << " Rnz = " << Rnz << ", Lnz = " << Lnz
...
double ForrestTomlin::_fill_factor() const { return fill_factor_; }

double ForrestTomlin::_pivottol() const { return pivottol_; }

void ForrestTomlin::_pivottol(double new_pivottol) { pivottol_ = new_pivottol; }
...
void ForrestTomlin::ComputeSpike(Int nb, const Int* bi, const double* bx) {
  ...
  // Solve L*lhs=b.
  work_ = 0.0;
  for (Int p = 0; p < nb; p++) work_[rowperm_inv_[bi[p]]] = bx[p];
  ...
  for (Int p = 0; p < dim_ + num_updates; p++) {
    if (work_[p] != 0.0) U_.push_back(p, work_[p]);
  }
  have_ftran_ = true;
}
...
void ForrestTomlin::ComputeEta(Int j) {
...
#ifndef NDEBUG
  for (Int k = 0; k < num_updates; k++) assert(work_[replaced_[k]] == 0.0);
#endif
  ...
  for (Int i = pos + 1; i < dim_ + num_updates; i++) {
    if (work_[i] != 0.0) R_.push_back(i, -work_[i] / pivot);
  }
  have_btran_ = true;
  replace_next_ = pos;
}
