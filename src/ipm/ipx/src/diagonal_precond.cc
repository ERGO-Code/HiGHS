...
void DiagonalPrecond::Factorize(const double* W, Info* info) {
  ...
  if (W) {
    for (Int i = 0; i < m; i++) diagonal_[i] = W[n + i];
    for (Int j = 0; j < n; j++) {
      double w = W[j];
...
double DiagonalPrecond::time() const { return time_; }

void DiagonalPrecond::reset_time() { time_ = 0.0; }
...
void DiagonalPrecond::_Apply(const Vector& rhs, Vector& lhs,
  ...
  if (rhs_dot_lhs) *rhs_dot_lhs = rldot;
  time_ += timer.Elapsed();
}
