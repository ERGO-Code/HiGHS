...
double NormalMatrix::time() const { return time_; }

void NormalMatrix::reset_time() { time_ = 0.0; }

void NormalMatrix::_Apply(const Vector& rhs, Vector& lhs, double* rhs_dot_lhs) {
  ...
  if (W_) {
#if MATVECMETHOD == 1
    for (Int i = 0; i < m; i++) lhs[i] = rhs[i] * W_[n + i];
    for (Int j = 0; j < n; j++) {
      Int begin = Ap[j], end = Ap[j + 1];
      double d = 0.0;
      for (Int p = begin; p < end; p++) d += rhs[Ai[p]] * Ax[p];
      d *= W_[j];
      for (Int p = begin; p < end; p++) lhs[Ai[p]] += d * Ax[p];
    }
#elif MATVECMETHOD == 2
    for (Int j = 0; j < n; j++) {
      Int begin = Ap[j], end = Ap[j + 1];
    ...
    for (Int i = 0; i < m; i++) {
      ...
      double d = 0.0;
      for (Int p = begin; p < end; p++) d += work_[Ati[p]] * Atx[p];
      lhs[i] += d;
    }
#elif MATVECMETHOD == 3
    for (Int j = 0; j < n; j++) {
