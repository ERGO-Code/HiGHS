...
void KKTSolverBasis::_Factorize(Iterate* iterate, Info* info) {
  ...
  for (Int j = 0; j < n + m; j++) colscale_[j] = iterate->ScalingFactor(j);
...
void KKTSolverBasis::_Solve(const Vector& a, const Vector& b, double tol,
  ...
  if (num_free > 0) basis_.SolveDense(work, work, 'T');
  ...
  for (Int k = 0; k < m; k++) work[k] = rhs[colperm[k]];
  ...
  for (Int k = 0; k < m; k++) y[colperm[k]] = lhs[k];
  ...
  for (Int p = 0; p < m; p++) x[basis_[p]] = work[p];
}
...
void KKTSolverBasis::DropPrimal(Iterate* iterate, Info* info) {
  ...
  for (Int p = 0; p < m; p++) {
    ...
    if (xj < 0.01 * zj && xj <= drop_primal) candidates.push_back(jb);
  }
  ...
  while (!candidates.empty()) {
    ...
    if (jmax >= 0) {
      ...
      bool exchanged;
      info->errflag = basis_.ExchangeIfStable(jb, jmax, pivot, 1, &exchanged);
      if (info->errflag) return;
...
void KKTSolverBasis::DropDual(Iterate* iterate, Info* info) {
  ...
  for (Int jn = 0; jn < n + m; jn++) {
    if (basis_.StatusOf(jn) != Basis::NONBASIC) continue;
    ...
    if (zj < 0.01 * xj && zj <= drop_dual) candidates.push_back(jn);
  }
  if (candidates.empty()) return;
  ...
  while (!candidates.empty()) {
    ...
    if (pmax >= 0) {
      ...
      bool exchanged;
      info->errflag = basis_.ExchangeIfStable(jb, jn, pivot, -1, &exchanged);
      if (info->errflag) return;
      if (!exchanged)  // factorization was unstable, try again
        continue;
