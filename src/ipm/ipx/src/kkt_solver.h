...
class KKTSolver {
 public:
  ...
  void Solve(const Vector& a, const Vector& b, double tol, Vector& x, Vector& y,
             Info* info);
 ...
 private:
  ...
  virtual void _Solve(const Vector& a, const Vector& b, double tol, Vector& x,
                      Vector& y, Info* info) = 0;
