...
void ConjugateResiduals::Solve(LinearOperator& C, const Vector& rhs, double tol,
                               const double* resscale, Int maxiter,
                               Vector& lhs) {
  ...
  while (true) {
    ...
    if (iter_ == maxiter) {
      control_.Debug(3) << " CR method not converged in " << maxiter
                        << " iterations."
...
void ConjugateResiduals::Solve(LinearOperator& C, LinearOperator& P,
  ...
  while (true) {
    ...
    if (resnorm <= tol) break;
    if (iter_ == maxiter) {
      control_.Debug(3) << " PCR method not converged in " << maxiter
                        << " iterations."
                        << " residual = " << sci2(resnorm) << ','
                        << " tolerance = " << sci2(tol) << '\n';
      errflag_ = IPX_ERROR_cr_iter_limit;
    ...
    if (cdot <= 0.0) {
      control_.Debug(3) << " matrix in PCR method not posdef. cdot = "
                        << sci2(cdot)
    ...
    if (iter_ % 5 == 0) {
      ...
      if (rsdot >= resnorm_precond_system) {
        control_.Debug(3) << " resnorm_precond_system old = "
                          << sci2(resnorm_precond_system) << '\n'
                          << " resnorm_precond_system new = " << sci2(rsdot)
                          << '\n';
