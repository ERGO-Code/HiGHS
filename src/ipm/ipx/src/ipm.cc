...
struct IPM::Step {
  Step(Int m, Int n)
      : x(n + m), xl(n + m), xu(n + m), y(m), zl(n + m), zu(n + m) {}
...
void IPM::ComputeStartingPoint() {
  ...
  for (Int j = 0; j < n + m; j++) {
    ...
    if (xj > ub[j]) xj = ub[j];
    x[j] = xj;
    if (xj != 0.0) ScatterColumn(AI, j, -xj, rb);
  }
  ...
  if (cnorm == 0.0) {
    ...
    for (Int j = 0; j < n + m; j++) {
      if (std::isfinite(lb[j])) zl[j] += zshift1;
      if (std::isfinite(ub[j])) zu[j] += zshift1;
    }
  }
  ...
  xu += xshift2;
  for (Int j = 0; j < n + m; j++) {
    if (std::isfinite(lb[j])) zl[j] += zshift2;
    if (std::isfinite(ub[j])) zu[j] += zshift2;
  }
...
static double StepToBoundary(const Vector& x, const Vector& dx,
  ...
  assert(alpha >= 0.0);
  if (blocking_index) *blocking_index = iblock;
  return alpha;
}
...
void IPM::SolveNewtonSystem(const double* rb, const double* rc,
                            ...
                            const double* sl, const double* su, Step& step) {
  const Model& model = iterate_->model();
  const Int m = model.rows();
  const Int n = model.cols();
  const SparseMatrix& AI = model.AI();
  ...
  if (rc) {
    for (Int j = 0; j < n + m; j++) rhs1[j] = -rc[j];
  }
  for (Int j = 0; j < n + m; j++) {
    double rlj = rl ? rl[j] : 0.0;
    double ruj = ru ? ru[j] : 0.0;
    if (iterate_->has_barrier_lb(j)) rhs1[j] += (sl[j] + zl[j] * rlj) / xl[j];
    if (iterate_->has_barrier_ub(j)) rhs1[j] -= (su[j] - zu[j] * ruj) / xu[j];
    if (iterate_->StateOf(j) == Iterate::State::fixed) rhs1[j] = 0.0;
  }
  assert(AllFinite(rhs1));
  if (rb) std::copy(rb, rb + m, std::begin(rhs2));
...
void IPM::PrintHeader() {
  control_.Log() << " " << Format("Iter", 4) << "  " << Format("P.res", 8)
                 << " " << Format("D.res", 8) << "  " << Format("P.obj", 15)
                 << " " << Format("D.obj", 15) << "  " << Format("mu", 8)
                 << "  " << Format("Time", 7);
  control_.Debug() << "  " << Format("stepsizes", 9) << "  "
...
void IPM::PrintOutput() {
  ...
  control_.Log() << " " << Format(info_->iter, 3) << (ipm_optimal ? "*" : " ")
  ...
  control_.Debug() << "  " << Fixed(step_primal_, 4, 2) << " "
                   << Fixed(step_dual_, 4, 2) << "  "
                   << Format(kkt_->basis_changes(), 7) << " "
                   << Format(kkt_->iter(), 7);
  control_.Debug() << "  " << Format(info_->dual_dropped, 7) << " "
                   << Format(info_->primal_dropped, 7);

  const Basis* basis = kkt_->basis();
  if (basis) {
    if (control_.Debug(4)) {
      control_.Debug(4) << "  " << Scientific(basis->MinSingularValue(), 9, 2);
      Timer timer;
