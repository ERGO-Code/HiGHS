...
static void PostprocessDependencies(Iterate* iterate, Basis* p_basis,
  ...
  if (info->dependent_cols > 0) {
    ...
    for (Int p = 0; p < m; p++) dx[basis[p]] = dxbasic[p];
  }
  ...
  iterate->Update(1.0, &dx[0], nullptr, nullptr, 1.0, &dy[0], nullptr, nullptr);
