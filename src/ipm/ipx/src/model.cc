...
void Model::Load(const Control& control, Int num_constr, Int num_var,
  ...
  if (dualize < 0) dualize = num_constr > 2 * num_var;
  if (dualize)
    LoadDual();
  else
    LoadPrimal();
  ...
  for (double x : lb_)
    if (std::isfinite(x)) norm_bounds_ = std::max(norm_bounds_, std::abs(x));
  for (double x : ub_)
    if (std::isfinite(x)) norm_bounds_ = std::max(norm_bounds_, std::abs(x));
...
void Model::PresolveStartingPoint(const double* x_user,
  ...
  if (x_user) std::copy_n(x_user, num_var_, std::begin(x_temp));
  if (slack_user) std::copy_n(slack_user, num_constr_, std::begin(slack_temp));
  if (y_user) std::copy_n(y_user, num_constr_, std::begin(y_temp));
  if (z_user) std::copy_n(z_user, num_var_, std::begin(z_temp));
  ScaleBasicSolution(x_temp, slack_temp, y_temp, z_temp);
  DualizeBasicSolution(x_temp, slack_temp, y_temp, z_temp, x_solver, y_solver,
                       z_solver);
  (void)(n);
  (void)(m);
}

void Model::PostsolveInteriorSolution(
  ...
  ScaleBackInteriorSolution(x_temp, xl_temp, xu_temp, slack_temp, y_temp,
                            zl_temp, zu_temp);
  if (x_user) std::copy(std::begin(x_temp), std::end(x_temp), x_user);
  if (xl_user) std::copy(std::begin(xl_temp), std::end(xl_temp), xl_user);
  if (xu_user) std::copy(std::begin(xu_temp), std::end(xu_temp), xu_user);
  if (slack_user)
    std::copy(std::begin(slack_temp), std::end(slack_temp), slack_user);
  if (y_user) std::copy(std::begin(y_temp), std::end(y_temp), y_user);
  if (zl_user) std::copy(std::begin(zl_temp), std::end(zl_temp), zl_user);
  if (zu_user) std::copy(std::begin(zu_temp), std::end(zu_temp), zu_user);
  (void)(n);
  (void)(m);
}

void Model::EvaluateInteriorSolution(
    const Vector& x_solver, const Vector& xl_solver, const Vector& xu_solver,
    const Vector& y_solver, const Vector& zl_solver, const Vector& zu_solver,
    Info* info) const {
  const Int m = rows();
  ...
  for (Int j = 0; j < num_var_; j++) {
    if (std::isfinite(scaled_lbuser_[j])) complementarity += xl[j] * zl[j];
    if (std::isfinite(scaled_ubuser_[j])) complementarity += xu[j] * zu[j];
  }
  for (Int i = 0; i < num_constr_; i++) complementarity -= y[i] * slack[i];
...
void Model::PostsolveBasicSolution(const Vector& x_solver,
  ...
  if (slack_user)
    std::copy(std::begin(slack_temp), std::end(slack_temp), slack_user);
  if (y_user) std::copy(std::begin(y_temp), std::end(y_temp), y_user);
  if (z_user) std::copy(std::begin(z_temp), std::end(z_temp), z_user);
  (void)(n);
  (void)(m);
}
...
void Model::EvaluateBasicSolution(const Vector& x_solver,
  ...
  for (Int j = 0; j < num_var_; j++) {
    if (x[j] < scaled_lbuser_[j]) xinfeas[j] = x[j] - scaled_lbuser_[j];
    if (x[j] > scaled_ubuser_[j]) xinfeas[j] = x[j] - scaled_ubuser_[j];
    if (vbasis[j] != IPX_nonbasic_lb && z[j] > 0.0) zinfeas[j] = z[j];
    if (vbasis[j] != IPX_nonbasic_ub && z[j] < 0.0) zinfeas[j] = z[j];
  }
  for (Int i = 0; i < num_constr_; i++) {
    if (constr_type_[i] == '<') {
      if (slack[i] < 0.0) sinfeas[i] = slack[i];
      if (y[i] > 0.0) yinfeas[i] = y[i];
    }
    if (constr_type_[i] == '>') {
      if (slack[i] > 0.0) sinfeas[i] = slack[i];
      if (y[i] < 0.0) yinfeas[i] = y[i];
...
static int CheckVectors(Int m, Int n, const double* rhs,
                        const char* constr_type, const double* obj,
                        const double* lb, const double* ub) {
  for (Int i = 0; i < m; i++)
    if (!std::isfinite(rhs[i])) return -1;
  for (Int j = 0; j < n; j++)
    if (!std::isfinite(obj[j])) return -2;
  for (Int j = 0; j < n; j++) {
    if (!std::isfinite(lb[j]) && lb[j] != -INFINITY) return -3;
    if (!std::isfinite(ub[j]) && ub[j] != INFINITY) return -3;
    if (lb[j] > ub[j]) return -3;
  }
  for (Int i = 0; i < m; i++)
    if (constr_type[i] != '=' && constr_type[i] != '<' && constr_type[i] != '>')
      return -4;
  return 0;
}
...
Int CheckMatrix(Int m, Int n, const Int* Ap, const Int* Ai, const double* Ax) {
  if (Ap[0] != 0) return -5;
  for (Int j = 0; j < n; j++)
    if (Ap[j] > Ap[j + 1]) return -5;
  for (Int p = 0; p < Ap[n]; p++)
    if (!std::isfinite(Ax[p])) return -6;
...
void Model::CopyInput(Int num_constr, Int num_var, const Int* Ap, const Int* Ai,
  ...
  if (CheckVectors(num_constr, num_var, rhs, constr_type, obj, lbuser,
                   ubuser) != 0) {
  ...
  for (Int j = 0; j < num_var; j++) {
    if (std::isinf(lbuser[j]) && std::isinf(ubuser[j])) num_free_var_++;
  ...
  for (double x : scaled_lbuser_)
    if (std::isfinite(x)) norm_rhs_ = std::max(norm_rhs_, std::abs(x));
...
void Model::ScaleModel(const Control& control) {
  ...
  if (control.scale() >= 1) EquilibrateMatrix();
...
void Model::LoadDual() {
  ...
  Int put = 0;
  for (double x : scaled_rhs_) c_[put++] = -x;
  for (double x : scaled_ubuser_)
    if (std::isfinite(x)) c_[put++] = x;
  ...
  for (Int i = 0; i < num_constr_; i++) switch (constr_type_[i]) {
      case '=':
        lb_[i] = -INFINITY;
        ub_[i] = INFINITY;
        break;
...
static double EquilibrationFactor(int expmin, int expmax, int exp) {
  if (exp < expmin) return std::ldexp(1.0, (expmin - exp + 1) / 2);
  if (exp > expmax) return std::ldexp(1.0, -((exp - expmax + 1) / 2));
  return 1.0;
}
...
void Model::FindDenseColumns() {
  ...
  for (Int j = 0; j < num_cols_; j++) colcount[j] = AI_.end(j) - AI_.begin(j);
...
void Model::PrintCoefficientRange(const Control& control) const {
  ...
  if (amin == INFINITY)  // no nonzero entries in A_
    amin = 0.0;
  control.Log() << Textline("Matrix range:") << "[" << Scientific(amin, 5, 0)
                << ", " << Scientific(amax, 5, 0) << "]\n";
  ...
  if (rhsmin == INFINITY)  // no nonzero entries in rhs
    rhsmin = 0.0;
  control.Log() << Textline("RHS range:") << "[" << Scientific(rhsmin, 5, 0)
  ...
  if (objmin == INFINITY)  // no nonzero entries in obj
    objmin = 0.0;
  control.Log() << Textline("Objective range:") << "["
                << Scientific(objmin, 5, 0) << ", " << Scientific(objmax, 5, 0)
                << "]\n";
  ...
  if (boundmin == INFINITY)  // no finite nonzeros entries in bounds
    boundmin = 0.0;
  control.Log() << Textline("Bounds range:") << "["
...
void Model::PrintPreprocessingLog(const Control& control) const {
  ...
  if (colscale_.size() > 0) {
    auto minmax =
        std::minmax_element(std::begin(colscale_), std::end(colscale_));
    minscale = std::min(minscale, *minmax.first);
    maxscale = std::max(maxscale, *minmax.second);
  }
  if (rowscale_.size() > 0) {
    auto minmax =
        std::minmax_element(std::begin(rowscale_), std::end(rowscale_));
    minscale = std::min(minscale, *minmax.first);
    maxscale = std::max(maxscale, *minmax.second);
  }
  if (minscale == INFINITY) minscale = 1.0;
  if (maxscale == 0.0) maxscale = 1.0;

  control.Log() << "Preprocessing\n"
                << Textline("Dualized model:") << (dualized() ? "yes" : "no")
                << '\n'
                << Textline("Number of dense columns:") << num_dense_cols()
                << '\n';
  if (control.scale() > 0) {
    control.Log() << Textline("Range of scaling factors:") << "["
...
void Model::ScaleBasicSolution(Vector& x, Vector& slack, Vector& y,
                               Vector& z) const {
  if (colscale_.size() > 0) {
    x /= colscale_;
    z *= colscale_;
...
void Model::ScaleBackResiduals(Vector& rb, Vector& rc, Vector& rl,
  ...
  if (rowscale_.size() > 0) rb /= rowscale_;
  for (Int j : flipped_vars_) {
    rc[j] *= -1.0;
    assert(ru[j] == 0.0);
    ru[j] = -rl[j];
...
void Model::ScaleBackBasis(std::vector<Int>& cbasis,
  ...
  for (Int j : flipped_vars_) {
    ...
    if (vbasis[j] == IPX_nonbasic_lb) vbasis[j] = IPX_nonbasic_ub;
  }
}

void Model::DualizeBasicSolution(const Vector& x_user, const Vector& slack_user,
  ...
  if (dualized_) {
    ...
    y_solver = -x_user;
    for (Int i = 0; i < num_constr_; i++) z_solver[i] = -slack_user[i];
    for (Int k = 0; k < (Int)boxed_vars_.size(); k++) {
      Int j = boxed_vars_[k];
      z_solver[num_constr_ + k] = c(num_constr_ + k) + y_solver[j];
    }
    for (Int i = 0; i < m; i++) z_solver[n + i] = c(n + i) - y_solver[i];
...
void Model::DualizeBackInteriorSolution(
    const Vector& x_solver, const Vector& xl_solver, const Vector& xu_solver,
    const Vector& y_solver, const Vector& zl_solver, const Vector& zu_solver,
  ...
  if (dualized_) {
    ...
    for (Int j = 0; j < num_var_; j++)
      if (!std::isfinite(scaled_lbuser_[j])) zl_user[j] = 0.0;
    ...
    zu_user = 0.0;
    Int k = num_constr_;
    for (Int j : boxed_vars_) zu_user[j] = xl_solver[k++];
    assert(k == n);
    ...
    xu_user = INFINITY;
    k = num_constr_;
    for (Int j : boxed_vars_) xu_user[j] = zl_solver[k++];
    assert(k == n);
...
void Model::DualizeBackBasicSolution(const Vector& x_solver,
  ...
  if (dualized_) {
    ...
    assert(num_constr_ + (int)boxed_vars_.size() == n);
    x_user = -y_solver;
    for (Int i = 0; i < num_constr_; i++) slack_user[i] = -z_solver[i];
    std::copy_n(std::begin(x_solver), num_constr_, std::begin(y_user));
    std::copy_n(std::begin(x_solver) + n, num_var_, std::begin(z_user));
    Int k = num_constr_;
    for (Int j : boxed_vars_) z_user[j] -= x_solver[k++];
    assert(k == n);
  } else {
    assert(num_constr_ == m);
    assert(num_var_ == n);
    std::copy_n(std::begin(x_solver), num_var_, std::begin(x_user));
    std::copy_n(std::begin(x_solver) + n, num_constr_, std::begin(slack_user));
  ...
  if (dualized_) {
    ...
    for (Int j = 0; j < num_var_; j++) {
      ...
      if (basic_status_solver[n + j] == 0)
        vbasis_user[j] =
            std::isfinite(scaled_lbuser_[j]) ? IPX_nonbasic_lb : IPX_superbasic;
      else
    ...
    for (Int j = 0; j < num_var_; j++) vbasis_user[j] = basic_status_solver[j];
  }
  (void)(m);
}
...
void Model::CorrectScaledBasicSolution(Vector& x, Vector& slack, Vector& y,
  ...
  for (Int j = 0; j < num_var_; j++) {
    if (vbasis[j] == IPX_nonbasic_lb) x[j] = scaled_lbuser_[j];
    if (vbasis[j] == IPX_nonbasic_ub) x[j] = scaled_ubuser_[j];
    if (vbasis[j] == IPX_basic) z[j] = 0.0;
  }
  for (Int i = 0; i < num_constr_; i++) {
    if (cbasis[i] == IPX_nonbasic) slack[i] = 0.0;
    if (cbasis[i] == IPX_basic) y[i] = 0.0;
  }
}
...
double DualInfeasibility(const Model& model, const Vector& x, const Vector& z) {
  ...
  double infeas = 0.0;
  for (Int j = 0; j < (Int)x.size(); j++) {
    if (x[j] > lb[j]) infeas = std::max(infeas, z[j]);
    if (x[j] < ub[j]) infeas = std::max(infeas, -z[j]);
  }
  return infeas;
}
