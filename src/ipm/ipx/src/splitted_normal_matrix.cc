...
void SplittedNormalMatrix::Prepare(const Basis& basis, const double* colscale) {
  ...
  for (Int j = 0; j < n + m; j++)
    if (basis.StatusOf(j) == Basis::NONBASIC) nonbasic_vars.push_back(j);
  ...
  for (Int k = 0; k < m; k++) {
    Int p = colperm_[k];
    Int j = basis[p];
    if (basis.StatusOf(j) == Basis::BASIC_FREE) free_positions_.push_back(k);
  }
  prepared_ = true;
}

const Int* SplittedNormalMatrix::colperm() const { return colperm_.data(); }

double SplittedNormalMatrix::time_B() const { return time_B_; }

double SplittedNormalMatrix::time_Bt() const { return time_Bt_; }

double SplittedNormalMatrix::time_NNt() const { return time_NNt_; }
...
void SplittedNormalMatrix::_Apply(const Vector& rhs, Vector& lhs,
  ...
  lhs += rhs;
  for (Int i : free_positions_) lhs[i] = 0.0;
  if (rhs_dot_lhs) *rhs_dot_lhs = Dot(rhs, lhs);
}
