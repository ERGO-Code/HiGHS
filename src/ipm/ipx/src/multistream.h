...
class Multistream : public std::ostream {
 public:
  Multistream() : std::ostream(nullptr) { std::ostream::rdbuf(&mbuffer_); }
  ...
  void clear() { mbuffer_.clear(); }

 private:
  struct multibuffer : public std::streambuf {
    void add(std::streambuf* b) { buffers.push_back(b); }
    void clear() { buffers.clear(); }
    int overflow(int c) override {
      for (std::streambuf* b : buffers) b->sputc(c);
      return c;
    }
