...
static std::vector<std::vector<Int> > Blockperm(const SparseMatrix& AI,
                                                const std::vector<Int>& jmatch,
  ...
  for (Int i : istack) {
    if (marked[i] != 2) {
      ...
      assert(top < end);
      blocks.push_back(
          std::vector<Int>(rowperm.begin() + top, rowperm.begin() + end));
    }
  }
  assert(top == 0);
...
static SparseMatrix CoarsenedGraph(
    const SparseMatrix& BT, const std::vector<std::vector<Int> >& blocks) {
  const Int m = BT.rows();
  ...
  for (Int i = 0; i < m; i++) assert(map2block[i] >= 0);
...
void SymbolicInvert(const Model& model, const std::vector<Int>& basis,
  ...
  if (rowcounts) {
    ...
    for (Int b = 0; b < nb; b++) {
      ...
      Int nz = 0;
      for (Int t = top; t < nb; t++) nz += blocks[stack[t]].size();
      for (Int i : blocks[b]) jcount[jmatch[i]] = nz;
    }
    for (Int p = 0; p < m; p++) {
  ...
  if (colcounts) {
    ...
    for (Int b = 0; b < nb; b++) {
      ...
      Int nz = 0;
      for (Int t = top; t < nb; t++) nz += blocks[stack[t]].size();
      for (Int i : blocks[b]) colcounts[i] = nz;
    }
  }
}
