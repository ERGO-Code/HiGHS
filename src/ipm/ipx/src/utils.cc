...
double Onenorm(const Vector& x) {
  double norm = 0.0;
  for (double xi : x) norm += std::abs(xi);
  return norm;
}
...
double Infnorm(const Vector& x) {
  double norm = 0.0;
  for (double xi : x) norm = std::max(norm, std::abs(xi));
  return norm;
}
...
double Dot(const Vector& x, const Vector& y) {
  ...
  double d = 0.0;
  for (Int i = 0; i < (Int)x.size(); i++) d += x[i] * y[i];
  return d;
}
...
void Permute(const std::vector<Int>& permuted_index, const Vector& rhs,
  ...
  for (Int i = 0; i < m; i++) lhs[permuted_index[i]] = rhs[i];
}

void PermuteBack(const std::vector<Int>& permuted_index, const Vector& rhs,
                 Vector& lhs) {
  Int m = permuted_index.size();
  for (Int i = 0; i < m; i++) lhs[i] = rhs[permuted_index[i]];
}
...
std::vector<Int> InversePerm(const std::vector<Int>& perm) {
  ...
  for (Int i = 0; i < m; i++) invperm.at(perm[i]) = i;
  return invperm;
}
...
std::vector<Int> Sortperm(Int m, const double* values, bool reverse) {
  ...
  for (Int i = 0; i < m; i++) value_index[i] = std::make_pair(values[i], i);
  if (reverse)
    std::sort(value_index.begin(), value_index.end(), greater_or_equal);
  else
    std::sort(value_index.begin(), value_index.end());
  for (Int i = 0; i < m; i++) perm[i] = value_index[i].second;
  return perm;
}
