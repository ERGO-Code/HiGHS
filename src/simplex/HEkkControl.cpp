...
void HEkk::initialiseControl() {
  ...
  simplex_info_.average_log_low_dual_steepest_edge_weight_error = 0;
  simplex_info_.average_log_high_dual_steepest_edge_weight_error = 0;
}
...
void HEkk::assessDSEWeightError(const double computed_edge_weight,
  ...
  if (updated_edge_weight < computed_edge_weight) {
    ...
    simplex_info_.average_log_low_dual_steepest_edge_weight_error =
        0.99 * simplex_info_.average_log_low_dual_steepest_edge_weight_error +
        0.01 * log(weight_error);
  } else {
    // Updated weight is correct or high
    weight_error = updated_edge_weight / computed_edge_weight;
    simplex_info_.average_log_high_dual_steepest_edge_weight_error =
        0.99 * simplex_info_.average_log_high_dual_steepest_edge_weight_error +
        0.01 * log(weight_error);
  }
}
...
bool HEkk::switchToDevex() {
  ...
  if (costly_DSE_iteration) {
    ...
    int local_iteration_count = iteration_count_ - simplex_info_.control_iteration_count0;
    ...
    switch_to_devex =
        simplex_info_.allow_dual_steepest_edge_to_devex_switch &&
        (simplex_info_.num_costly_DSE_iteration >
	 local_iteration_count * costly_DSE_fraction_num_costly_DSE_iteration_before_switch) &&
        (local_iteration_count > costly_DSE_fraction_num_total_iteration_before_switch * local_num_tot);
