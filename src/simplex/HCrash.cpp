...
void HCrash::bixby(HModel *ptr_model, int Crash_Mode) {
  ...
  for (int ps_n = 0; ps_n < numCol; ps_n++) {
    ...
    //			Find out if there is some row l for which |entry| > bixby_mu_b *
    //v_l
#ifdef HiGHSDEV
      double rp_v;
#endif
      for (int el_n = Astart[c_n]; el_n < Astart[c_n + 1]; el_n++) {
        int r_n = Aindex[el_n];
        //				If this value in the column would give an
        //unacceptable multiplier then continue to next pass
...
#ifdef HiGHSDEV
      if (nx_ps && Rp_Bixby_Ps)
        printf(": Unacceptable multiplier of %g > %g\n", rp_v, bixby_mu_b);
#endif
    }
    //		Some value in the column would give an unacceptable multiplier so
    ...
    //		If there is no entry in a row with no pivot then continue to next
    //pass
    if (r_o_mx_aa == no_ix) {
#ifdef HiGHSDEV
...
bool HCrash::bixby_iz_da(HModel *ptr_model) {
  ...
  //for each row 	bixby_pv_in_r: 		"I" in the paper: this is a
  //0/1 flag to indicate whether there is a basic variable in each row
...
void HCrash::crsh_iz_vr_ty(HModel *ptr_model, int Crash_Mode) {
  ...
  // Initialise the counts of numbers and changes of variable types - just for
  // reporting
...
void HCrash::crsh_ck_an_impl_bd() {
  ...
  //		for (int c_n = 0; c_n < numCol; c_n++) {
  //			printf(
  //					"Col %5d has primal bounds [%12.4e,%12.4e] and implied
  //primal bounds [%12.4e,%12.4e]\n",
  //					c_n, colLower[c_n], colUpper[c_n],
  //					primalColLowerImplied[c_n],
  //primalColUpperImplied[c_n]);
  //		}
  //		for (int c_n = 0; c_n < numCol; c_n++) {
  //			printf(
  //					"Col %5d has primal bounds [%12.4e,%12.4e] and implied
  //dual bounds [%12.4e,%12.4e]\n",
  //					c_n, colLower[c_n], colUpper[c_n],
  //					dualColLowerImplied[c_n],
  //dualColUpperImplied[c_n]);
  //		}
  //		for (int r_n = 0; r_n < numRow; r_n++) {
  //			printf(
  //					"Row %5d has primal bounds [%12.4e,%12.4e] and implied
  //primal bounds [%12.4e,%12.4e]\n",
  //					r_n, rowLower[r_n], rowUpper[r_n],
  //					primalRowLowerImplied[r_n],
  ...
  for (int c_n = 0; c_n < numCol; c_n++) {
    if (colLower[c_n] > -HSOL_CONST_INF) {
      //			Col lower >  -inf so dualColUpperImplied < inf
      //is tighter
      if (dualColUpperImplied[c_n] < HSOL_CONST_INF) {
        numTighterDualColUpper += 1;
      }
    } else {
      //			Col lower <= -inf so dualColUpperImplied should
      //be at most zero
    ...
    if (colUpper[c_n] < HSOL_CONST_INF) {
      //			Col upper <   inf so dualColLowerImplied > -inf
      //is tighter
      if (dualColLowerImplied[c_n] > -HSOL_CONST_INF) {
        numTighterDualColLower += 1;
      }
    } else {
      //			Col upper >=  inf so dualRowLowerImplied should
      //be at least zero
  ...
  for (int r_n = 0; r_n < numRow; r_n++) {
    if (rowLower[r_n] > -HSOL_CONST_INF) {
      //			Row lower >  -inf so dualRowUpperImplied < inf
      //is tighter
      if (dualRowUpperImplied[r_n] < HSOL_CONST_INF) {
        numTighterDualRowUpper += 1;
      }
    } else {
      //			Row lower <= -inf so dualRowUpperImplied should
      //be at most zero
    ...
    if (rowUpper[r_n] < HSOL_CONST_INF) {
      //			Row upper <   inf so dualColLowerImplied > -inf
      //is tighter
      if (dualRowLowerImplied[r_n] > -HSOL_CONST_INF) {
        numTighterDualRowLower += 1;
      }
    } else {
      //			Row upper >=  inf so dualColLowerImplied should
      //be at least zero
...
void HCrash::ltssf(HModel *ptr_model, int Crash_Mode) {
  ...
  //	for (int r_n = 0; r_n < numRow; r_n++)
  //		printf(
  //				"Row: %d; BaseIndex = %d; Work[L,V,U] = [%g,%g,%g];
  //Base[L,V,U] = [%g,%g,%g]\n", 				r_n,
  //model->getBaseIndex()[r_n], model->getWorkLower()[r_n],
  //				model->getWorkValue()[r_n],
  //model->getWorkUpper()[r_n],
  //				model->getBaseLower()[r_n],
  //model->getBaseValue()[r_n],
  //				model->getBaseUpper()[r_n]);
  //	for (int vr_n = 0; vr_n < numTot; vr_n++)
  //		printf("Var: %d; NonbasicFlag = %d; NonbasicMove = %d; Work[L,V,U] =
  //[%g,%g,%g]\n", vr_n,
  //				model->getNonbasicFlag()[vr_n],
  //				model->getNonbasicMove()[vr_n],
  //model->getWorkLower()[vr_n],
  //				model->getWorkValue()[vr_n],
  //model->getWorkUpper()[vr_n]);
  ...
  if ((!alw_al_bs_cg) && (mx_r_pri + mx_c_pri <= crsh_mx_pri_v)) {
    ...
    //crsh_r_ty_pri_v[crsh_r_ty[r_n]], crsh_r_k[r_n]);
    //	}
#ifdef HiGHSDEV
  if (ltssf_ck_fq > 0) {
  ...
  if (Crash_Mode == Crash_Mode_Bs) {
    const int *nonbasicFlag = model->getNonbasicFlag();
    for (int r_n = 0; r_n < numRow; r_n++) printf("Row %4d has nonbasic flag =
  %1d\n", r_n, nonbasicFlag[numCol+r_n]); for (int c_n = 0; c_n < numCol; c_n++)
...
#endif
    if (TwoD_hdr) {
      //			Determine whether the are still rows worth
      //removing
  ...
  if (crsh_fn_cf_pri_v > crsh_fn_cf_k) {
    ...
    if (OneD_hdr) {
      ...
      for (int pri_v = crsh_mx_pri_v; pri_v > crsh_mn_pri_v; pri_v--) {
        ...
        //     Note the limiting priority function value so that the search can
        //     be stopped if it is attained corresponds to a maximum priority
        int lm_r_pri_fn_v = crsh_fn_cf_pri_v * pri_v - crsh_fn_cf_k;
        do {
          int k = crsh_r_k[r_n];
          int r_pri_fn_v = crsh_fn_cf_pri_v * pri_v - crsh_fn_cf_k * k;
          if (r_pri_fn_v > mx_r_pri_fn_v) {
            cz_r_n = r_n;
            mx_r_pri_fn_v = r_pri_fn_v;
            if (mx_r_pri_fn_v == lm_r_pri_fn_v) break;
          }
    ...
    if (OneD_hdr) {
      ...
      for (int k = 1; k < numCol + 1; k++) {
        ...
        // Note the limiting priority function value so that the search can be
        // stopped if it is attained corresponds to a maximum priority
        int lm_r_pri_fn_v = crsh_fn_cf_pri_v * mx_r_pri_v - crsh_fn_cf_k * k;
        do {
...
void HCrash::ltssf_cz_c(HModel *ptr_model) {
  ...
  for (int el_n = CrshARstart[cz_r_n]; el_n < CrshARstart[cz_r_n + 1]; el_n++) {
    ...
    if (c_pri_fn_v > mx_c_pri_fn_v) {
      ...
      //				Matches the best column so far: possibly
      //break tie on cost
      double sense_col_cost = objSense * colCost[c_n];
...
void HCrash::ltssf_u_da(HModel *ptr_model) {
  ...
  //then get the new maximum row priority value
  if (OneD_hdr) {
    if ((crsh_r_pri_hdr[cz_r_pri_v] == no_lk) && (cz_r_pri_v == mx_r_pri_v)) {
      mx_r_pri_v = -HSOL_CONST_I_INF;
      for (int pri_v = crsh_mn_pri_v; pri_v < crsh_mx_pri_v + 1; pri_v++)
        if (crsh_r_pri_hdr[pri_v] != no_lk) mx_r_pri_v = pri_v;
    }
  }
  if (TwoD_hdr) {
...
void HCrash::ltssf_u_da_af_bs_cg(HModel *ptr_model) {
  ...
  for (int r_el_n = CrshARstart[cz_r_n]; r_el_n < CrshARstart[cz_r_n + 1];
    ...
    for (int el_n = Astart[c_n]; el_n < Astart[c_n + 1]; el_n++) {
      ...
      //     Remove the row from the linked list with this number of active
      //     entries
      int prev_r_n;
      ...
      if (OneD_hdr) {
        ...
        if (nx_r_n != no_lk) crsh_r_k_lkb[nx_r_n] = prev_r_n;
        //     Reduce the number of active entries in this row by one and...
        r_k -= 1;
        crsh_r_k[r_n] = r_k;
        if (r_k > 0) {
          //     ... either add the row as the header of the list with one fewer
          //     number of active entries...
          nx_r_n = crsh_r_k_hdr[r_k];
          crsh_r_k_hdr[r_k] = r_n;
          crsh_r_k_lkb[r_n] = no_lk;
          crsh_r_k_lkf[r_n] = nx_r_n;
          if (nx_r_n != no_lk) crsh_r_k_lkb[nx_r_n] = r_n;
        } else {
          //     ...or, if the count is zero, the row leaves the active
          //     submatrix and the priority value data structure
          ...
          if (nx_r_n != no_lk) crsh_r_pri_lkb[nx_r_n] = prev_r_n;
        }
      }
      if (TwoD_hdr) {
        if (OneD_hdr) {
        ...
        if ((crsh_r_pri_k_hdr[hdr_ix] == no_lk) &&
          ...
          //					Set crsh_r_pri_mn_r_k to numCol + 1 in case
          //r_k=numCol so priority is cleared
          ...
          //					if (crsh_r_pri_mn_r_k[pri_v] == numCol + 1)
          //printf("Priority %d now has no rows\n", pri_v);
        }
        //     Reduce the number of active entries in this row by one and...
        r_k -= 1;
        crsh_r_k[r_n] = r_k;
        ...
        //     number of active entries...
          ...
          if (Rp_TwoD_da) {
            ltssf_rp_pri_k_da();
            printf(
                "2: Remove row %d of pri %d and count %d from active "
                "submatrix\n",
                r_n, pri_v, r_k);
          }
#endif
          ...
          //     submatrix...
          crsh_act_r[r_n] = crsh_vr_st_no_act;
          //     ... and has already left the priority value and count data
          //     structure
        }
      }
    }
...
void HCrash::ltssf_u_da_af_no_bs_cg() {
  ...
  for (int r_el_n = CrshARstart[cz_r_n]; r_el_n < CrshARstart[cz_r_n + 1];
       r_el_n++) {
    int c_n = CrshARindex[r_el_n];
    // printf("Deactivating chosen row %2d: Entry %4d is column %2d has activity
  ...
  if (TwoD_hdr) {
    //	     Remove the row from the linked list with this priority and number
    //of active entries
    crsh_act_r[r_n] = crsh_vr_st_no_act;
    int pri_v = crsh_r_ty_pri_v[crsh_r_ty[r_n]];
    int r_k = crsh_r_k[r_n];
    ...
    if ((crsh_r_pri_k_hdr[hdr_ix] == no_lk) &&
      ...
      // highest  Set crsh_r_pri_mn_r_k to numCol + 1 in case r_k=numCol so
      // priority is cleared
      crsh_r_pri_mn_r_k[pri_v] = numCol + 1;
      for (int qy_k = r_k + 1; qy_k < numCol + 1; qy_k++) {
        int hdr_ix = pri_v * (numCol + 1) + qy_k;
      ...
      // if (crsh_r_pri_mn_r_k[pri_v] == numCol + 1)	printf("Priority %d
      // now has no rows\n", pri_v);
    }
  }
}
...
void HCrash::ltssf_iz_da(HModel *ptr_model, int Crash_Mode) {
  ...
  if (Crash_Mode == Crash_Mode_Bs) {
    ...
    model->matrix.setup_lgBs(numCol, numRow, &Astart[0], &Aindex[0],
                             &Avalue[0]);
  ...
  if (OneD_hdr) {
    ...
    for (int c_n = 0; c_n < numCol + 1; c_n++) crsh_r_k_hdr[c_n] = no_lk;
  }
  if (TwoD_hdr) {
  ...
  for (int r_n = 0; r_n < numRow; r_n++) {
    ...
    if (pri_v == crsh_no_act_pri_v) {
      //	Rows with no priority value are free - and should be removed by
      //presolve 	They will be basic
      crsh_act_r[r_n] = crsh_vr_st_no_act;
    } else {
  ...
  for (int c_n = 0; c_n < numCol; c_n++) {
    ...
    if ((pri_v == crsh_no_act_pri_v) || (c_n_en == 0)) {
      //	Columns with no priority value are fixed or zero - and should be
      //removed by presolve 	They will be nonbasic
      crsh_act_c[c_n] = crsh_vr_st_no_act;
    } else {
    ...
    if (crsh_act_c[c_n] == crsh_vr_st_act) {
      ...
      //			update the number of active entries in those
      //rows
      //
      //			Initialise the count of active entries in the
      //row
      crsh_c_k[c_n] = 0;
      for (int el_n = Astart[c_n]; el_n < Astart[c_n + 1]; el_n++) {
        int r_n = Aindex[el_n];
    ...
    // Error surely, since it ends up doubling hte numbers of columns of each
    // type
  ...
  for (int r_n = 0; r_n < numRow; r_n++) {
    ...
    if (OneD_hdr) {
      //		Add as the header of the appropriate list of rows with a
      //given count
  ...
  for (int c_n = 0; c_n < numCol; c_n++) {
    ...
    for (int el_n = Astart[c_n]; el_n < Astart[c_n + 1]; el_n++) {
      int r_n = Aindex[el_n];
      //	    	printf("Col %d; Row %d: Activity %d;  CrshARstart =
      //%d; |Value| = %g Max = %g\n", c_n, r_n,
      //crsh_act_r[r_n],CrshARstart[r_n], abs(Avalue[el_n]),
      //crsh_mtx_c_mx_abs_v[c_n]);
      crsh_mtx_c_mx_abs_v[c_n] =
          max(abs(Avalue[el_n]), crsh_mtx_c_mx_abs_v[c_n]);
      if (crsh_act_r[r_n] == crsh_vr_st_no_act) continue;
  ...
  //	printf("\nCol-wise matrix:\n");
  //	for (int c_n = 0; c_n < numCol; c_n++) {
  //		printf("Col %2d (%3d->-%3d): ", c_n, Astart[c_n], Astart[c_n +
  //1]-1); 		for (int el_n = Astart[c_n]; el_n < Astart[c_n + 1];
  //el_n++) { 			int r_n = Aindex[el_n];
  //			printf("%2d ", r_n);
  //		}
  //		printf("\n");
  //	}
  //	printf("\nRow-wise matrix:\n");
  //	for (int r_n = 0; r_n < numRow; r_n++) {
  //		printf("Row %2d (%3d->-%3d): ", r_n, CrshARstart[r_n],
  //CrshARstart[r_n + 1]-1); 		for (int el_n = CrshARstart[r_n]; el_n <
  //CrshARstart[r_n + 1]; el_n++) { 			int c_n =
  //CrshARindex[el_n]; 			printf("%2d ", c_n);
  //		}
  //		printf("\n");
  //	}
  ...
  if (OneD_hdr) {
    for (int pri_v = crsh_mn_pri_v; pri_v < crsh_mx_pri_v + 1; pri_v++) {
      ...
      do {
        ...
        if (ck_pri_v != pri_v) {
          er_fd = true;
          printf("ERROR: Row %d: Type %d has priority value error %d; %d\n",
        ...
        if (nx_r_n != no_lk) {
          if (crsh_r_pri_lkb[nx_r_n] != r_n) {
            er_fd = true;
            printf("ERROR: Row %d: Type %d has back link error %d; %d\n", r_n,
                   r_ty, nx_r_n, crsh_r_pri_lkf[r_n]);
          }
        }
        r_n = nx_r_n;
      } while (r_n != no_lk);
      ...
      if (r_n != no_lk) do {
          ...
          if (nx_r_n != no_lk) {
            if (crsh_r_k_lkb[nx_r_n] != r_n) {
              er_fd = true;
              printf(
                  "ERROR: Row %d: Count %d has back link error nx_r_n = %d; "
                  "crsh_r_k_lkf[r_n] = %d\n",
  ...
  if (TwoD_hdr) {
    ...
    for (int pri_v = crsh_mn_pri_v; pri_v < crsh_mx_pri_v + 1; pri_v++) {
        ...
        do {
          ...
          if (nx_r_n != no_lk) {
            ...
            if (prev_nx_r_n != r_n) {
              er_fd = true;
              printf(
                  "ERROR: Back link error for nx_r_n = %d: prev_nx_r_n = %d "
                  "but r_n = %d\n",
                  nx_r_n, prev_nx_r_n, r_n);
            }
          }
          //					Update the true minimum row
          //count
          mn_r_k = min(k, mn_r_k);
          r_n = nx_r_n;
        } while (r_n != no_lk);
      ...
      if (crsh_r_pri_mn_r_k[pri_v] != mn_r_k)
        printf(
            "ERROR: Priority %d has crsh_r_pri_mn_r_k = %d < %d = true "
            "mn_r_k\n",
