...
void HMatrix::price_by_row_w_sw(HVector &row_ap, HVector &row_ep,
  ...
  if (hist_dsty <= hyperPRICE) {
    for (int i = nx_i; i < ep_count; i++) {
      ...
      if (rpRow) {
        printf("Hyper_p Row %1d: multi = %g; NNz = %d\n", i, multi, iRowNNz);
        fflush(stdout);
      }
...
void HMatrix::price_by_row_no_index(HVector &row_ap, HVector &row_ep,
  ...
  for (int i = fm_i; i < ep_count; i++) {
    ...
    double multi = ep_array[iRow];
    if (rpRow) {
      printf("StdRowPRICE Row %1d: multi = %g; NNz = %d\n", i, multi, iRowNNz);
      fflush(stdout);
    }
    for (int k = ARstart[iRow]; k < AR_Nend[iRow]; k++) {
      int index = ARindex[k];
...
void HMatrix::price_by_row_ultra(HVector &row_ap, HVector &row_ep) const {
  ...
  if (fm_i < ep_count) {
    ...
    if (rpRow) {
      printf(
          "PRICE is not complete for %d = fm_i < ep_count = %d; ap_pWd = %1d\n",
          fm_i, ep_count, ap_pWd);
      fflush(stdout);
    }
    if (ap_pWd == row_ap.dfSparseDaStr) {
      const double hist_dsty =
          -0.1;  // Historical density always forces hyper-sparse Price
      const double sw_dsty =
          price_by_row_sw_dsty;  // Default switch to standard row-wise price
      price_by_row_w_sw(row_ap, row_ep, hist_dsty, fm_i, sw_dsty);
      //      price_by_row_no_index(row_ap, row_ep, fm_i);
    } else {
    ...
    if (rpRow) {
      printf("PRICE is complete with ap_pWd = %1d\n", ap_pWd);
      fflush(stdout);
    }
    price_by_row_rm_cancellation(row_ap);
  }
  //  row_ap.pWd = ap_pWd;
  //  row_ap.count = ap_count;
  //  printf("Ultra PRICE completion: Set  row_ap.pWd = %d; row_ap.count =
  //  %d\n", ap_pWd, ap_count);
}
...
void HMatrix::price_by_row_ultra0(HVector &row_ap, HVector &row_ep,
  ...
  if (!row_apPackMap.empty()) {
    printf("ERROR: row_apPackMap not empty\n");
    fflush(stdout);
  }
  int fm_i = *fm_i_;
  int nx_i = fm_i;
  for (int i = fm_i; i < ep_count; i++) {
    ...
    if (rpRow) {
      printf("Ultra-0 Row %1d: multi = %g; NNz = %d\n", i, multi, iRowNNz);
      fflush(stdout);
    }
    ...
    for (int k = ARstart[iRow]; k < AR_Nend[iRow]; k++) {
      ...
      if (rpOps) {
        printf(
            "Entry %5d: ix %4d; v %11.4g; ixEn %4d; ap_c = %6d; PV2 = %11.4g",
            k, index, ARvalue[k], indexEn, ap_count, itPackMap->second);
        fflush(stdout);
      }
      ...
      if (rpPackmap) {
        ...
        int en = 0;
        for (itPackMap = row_apPackMap.begin();
             itPackMap != row_apPackMap.end(); ++itPackMap) {
  ...
  if (fm_i < ep_count) {
    if (rpRow) {
      printf("PRICE not complete with ap_pWd = %1d: %d = fm_i < ep_count = %d",
             ap_pWd, fm_i, ep_count);
      printf(
          "| ap_count = %d; iRowNNz = %d; ap_count+iRowNNz = %d >= %d = "
          "row_ap.packMapMxZ",
    ...
    for (itPackMap = row_apPackMap.begin(); itPackMap != row_apPackMap.end();
      ...
      ap_index[en++] = index;
      //      printf("Conversion: %3d sets ap_array[%6d] = %11.4g; ap_index[%3d]
  ...
  //  printf("Ultra12: setting row_ap.count = %d; row_ap.pWd = %d; *fm_i_ =
...
void HMatrix::price_by_row_ultra12(HVector &row_ap, HVector &row_ep,
  ...
  //  for (int index = 0; index < numCol; index++) {
  //    if (ap_valueP1[index] != ilP) {printf("ERROR: Initial ap_valueP1[%5d] =
  ...
  for (int i = fm_i; i < ep_count; i++) {
    ...
    iRowNNz = AR_Nend[iRow] - ARstart[iRow];
    if (rpRow) {
      printf("Ultra-1 Row %1d: multi = %g; NNz = %d\n", i, multi, iRowNNz);
      fflush(stdout);
    }
  ...
  if (fm_i < ep_count) {
    if (rpRow) {
      printf("PRICE not complete with ap_pWd = %1d: %d = fm_i < ep_count = %d",
             ap_pWd, fm_i, ep_count);
      printf("| ap_count = %d; iRowNNz = %d; ap_count+iRowNNz = %d >= %d = ilP",
             ap_count, iRowNNz, ap_count + iRowNNz, ilP);
      printf("\n");
    }
  ...
  if (fm_i < ep_count) {
    ...
    for (int i = fm_i; i < ep_count; i++) {
      ...
      iRowNNz = AR_Nend[iRow] - ARstart[iRow];
      if (rpRow) {
        printf("Ultra-2 Row %1d: multi = %g; NNz = %d\n", i, multi, iRowNNz);
        fflush(stdout);
      }
      if (ap_count + iRowNNz >= ilP) {
        nx_i = i;
    ...
    if (rpRow) {
      printf("Completed ultra-sparse PRICE with 2-byte pointers, \n");
      fflush(stdout);
    }
  }
  fm_i = nx_i;
  ...
  if (ap_pWd >= row_ap.p0SparseDaStr) {
    for (int en = 0; en < row_ap.count; en++) {
      ...
      if (ap_pWd >= row_ap.p1SparseDaStr) {
        ...
        if (pointer_er) {
          printf("PvR entry %2d: index = %2d, value = %11.4g", en, index,
                 value);
