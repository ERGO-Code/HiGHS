...
class SimplexTimer {
 public:
  ...
  void reportSimplexClockList(const char* grepStamp,
    ...
    printf(
        "SimplexTotalClock = %d; clock[SimplexTotalClock] = %d; "
        "timer.clock_time[clock[SimplexTotalClock]] = %g\n",
        SimplexTotalClock, clock[SimplexTotalClock],
        timer.clock_time[clock[SimplexTotalClock]]);
