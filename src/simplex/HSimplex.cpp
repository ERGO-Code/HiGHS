...
void options(HighsModelObject &highs_model_object, const HighsOptions &opt) {
  ...
  simplex_info.analyseLpSolution = useful_analysis;        // false;
...
void postsolveRefineHighsBasisAndSolution(HighsModelObject &highs_model_object) {
  ...
  for (int iVar=0; iVar<lp.numCol_+lp.numRow_; iVar++) {
    ...
    if (iVar < lp.numCol_) {
      int iCol = iVar;
      is_nonbasic = basis.col_status[iCol] != HighsBasisStatus::BASIC;
      lower = lp.colLower_[iCol];
      upper = lp.colUpper_[iCol];
      value = solution.col_value[iCol];
    } else {
      int iRow = iVar - lp.numCol_;
      is_nonbasic = basis.row_status[iRow] != HighsBasisStatus::BASIC;
...
void rebuildPostsolve(HighsModelObject &highs_model_object) {
  ...
  for (int iCol = 0; iCol < lp.numCol_; iCol++) {
    ...
    if (col_status[iCol] == HighsBasisStatus::BASIC) {
      ...
      basicIndex[num_basic_var] = iVar;
      assert(num_basic_var < lp.numRow_);
      if (num_basic_var == lp.numRow_) {
 ...
 //  reportSimplexLpStatus(highs_model_object.simplex_lp_status_, "In rebuildPostsolve");
  ...
  if (rebuild_postsolve_report_level>=0) {
	...
	if (rebuild_postsolve_report_level==2) {
	  ...
	  printf("%5d %5d %12g %12g %12g\n",
		 iRow, nonbasicFlag[iRow], new_row_dual, old_row_dual, dl_dual);
	}
      }
  ...
  // Analyse the differences in column duals
	...
	if (rebuild_postsolve_report_level==2) {
	  ...
	  header_written = true;
	  printf("%5d %5d %12g %12g %12g\n",
		 iCol, nonbasicFlag[iCol], new_col_dual, old_col_dual, dl_dual);
	}
      }
  ...
  for (int iRow = 0; iRow < lp.numRow_; iRow++) {
    if (!nonbasicFlag[lp.numCol_+iRow]) continue;
