...
void report_basis(HighsModelObject &highs_model_object) {
  ...
  printf("\nReporting current basis: solver_lp.numCol_ = %d; solver_lp.numRow_ "
         "= %d\n",
         solver_lp.numCol_, solver_lp.numRow_);
  if (solver_lp.numCol_ > 0)
...
void transpose_solver_lp(HighsModelObject &highs_model_object) {
  ...
  HighsSimplexLpStatus &simplex_lp_status = highs_model_object.simplex_lp_status_;
#ifdef HiGHSDEV
  printf("Called transpose_solver_lp: simplex_lp_status.is_transposed = "
         "%d\n",
         simplex_lp_status.is_transposed);
#endif
...
void scale_solver_lp(HighsModelObject &highs_model_object) {
  ...
  HighsSimplexLpStatus &simplex_lp_status = highs_model_object.simplex_lp_status_;
#ifdef HiGHSDEV
  printf("Called scale_solver_lp: simplex_lp_status.is_scaled = %d\n",
         simplex_lp_status.is_scaled);
#endif
...
void permute_solver_lp(HighsModelObject &highs_model_object) {
  ...
  HighsSimplexLpStatus &simplex_lp_status = highs_model_object.simplex_lp_status_;
#ifdef HiGHSDEV
  printf("Called permute_solver_lp: simplex_lp_status.is_permuted = %d\n",
         simplex_lp_status.is_permuted);
#endif
...
#ifdef HiGHSDEV
  printf(
      "Called tighten_solver_lp: simplex_lp_status.is_tightened = %d\n",
      simplex_lp_status.is_tightened);
#endif
...
void initialise_with_logical_basis(HighsModelObject &highs_model_object) {
  ...
  for (int row = 0; row < solver_lp.numRow_; row++)
    basis.basicIndex_[row] = solver_lp.numCol_ + row;
  for (int col = 0; col < solver_lp.numCol_; col++)
...
void extend_with_logical_basis(HighsModelObject &highs_model_object,
  ...
  // of rows. Also assumes that solver_lp_->numCol_ and solver_lp_->numRow_ have
  ...
  // firstcol = %d\n   lastcol =  %d\n   solver_lp.numRow_ =   %d\n   firstrow =
  // %d\n   lastrow = %d\n", solver_lp.numCol_, firstcol, lastcol,
  // solver_lp.numRow_, firstrow, lastrow); Determine the numbers of columns and
  // rows to be added
  ...
  if (rowShift > 0) {
    ...
    for (int row = local_oldNumRow - 1; row >= 0; row--) {
      ...
      basis.nonbasicFlag_[solver_lp.numCol_ + row] =
          basis.nonbasicFlag_[local_oldNumCol + row];
      basis.nonbasicMove_[solver_lp.numCol_ + row] =
          basis.nonbasicMove_[local_oldNumCol + row];

      simplex_info.workCost_[solver_lp.numCol_ + row] =
          simplex_info.workCost_[local_oldNumCol + row];
      simplex_info.workDual_[solver_lp.numCol_ + row] =
          simplex_info.workDual_[local_oldNumCol + row];
      simplex_info.workShift_[solver_lp.numCol_ + row] =
          simplex_info.workShift_[local_oldNumCol + row];

      simplex_info.workLower_[solver_lp.numCol_ + row] =
          simplex_info.workLower_[local_oldNumCol + row];
      simplex_info.workUpper_[solver_lp.numCol_ + row] =
          simplex_info.workUpper_[local_oldNumCol + row];
      simplex_info.workRange_[solver_lp.numCol_ + row] =
          simplex_info.workRange_[local_oldNumCol + row];
      simplex_info.workValue_[solver_lp.numCol_ + row] =
  ...
  // Check that columns 0..firstcol-1 and rows 0..firstrow-1 constitute a valid
  // basis.
  basisOK = nonbasic_flag_basic_index_ok(highs_model_object, solver_lp.numCol_,
                                         solver_lp.numRow_);
  assert(basisOK);
#endif
  ...
  if (numAddCol)
    update_solver_lp_status_flags(highs_model_object, LpAction::NEW_COLS);
  if (numAddRow)
    update_solver_lp_status_flags(highs_model_object, LpAction::NEW_ROWS);
}
...
void setup_for_solve(HighsModelObject &highs_model_object) {
  ...
  if (!(simplex_lp_status.has_matrix_col_wise &&
    ...
    if (simplex_info.num_basic_logicals == solver_num_row) {
      ...
      matrix.setup(solver_num_col, solver_num_row, &solver_lp.Astart_[0],
                   &solver_lp.Aindex_[0], &solver_lp.Avalue_[0],
  ...
  factor.setup(solver_num_col, solver_num_row, &solver_lp.Astart_[0],
               &solver_lp.Aindex_[0], &solver_lp.Avalue_[0],
               &basis.basicIndex_[0]);
      ...
      if (!highs_isInfinity(-simplex_info.workLower_[var])) {
        ...
        if (!ok) {
          printf("For col %d, simplex_info.workLower_ should be %g but is %g\n",
                 col, solver_lp.colLower_[col], simplex_info.workLower_[var]);
          return ok;
        }
      }
      if (!highs_isInfinity(simplex_info.workUpper_[var])) {
        ...
        if (!ok) {
          printf("For col %d, simplex_info.workUpper_ should be %g but is %g\n",
                 col, solver_lp.colUpper_[col], simplex_info.workUpper_[var]);
          return ok;
        }
      }
    }
      ...
      if (!highs_isInfinity(-simplex_info.workLower_[var])) {
        ...
        if (!ok) {
          printf("For row %d, simplex_info.workLower_ should be %g but is %g\n",
                 row, -solver_lp.rowUpper_[row], simplex_info.workLower_[var]);
          return ok;
        }
      }
      if (!highs_isInfinity(simplex_info.workUpper_[var])) {
        ...
        if (!ok) {
          printf("For row %d, simplex_info.workUpper_ should be %g but is %g\n",
                 row, -solver_lp.rowLower_[row], simplex_info.workUpper_[var]);
          return ok;
        }
      }
    }
  ...
  if (!simplex_info.costs_perturbed) {
    for (int col = 0; col < solver_lp.numCol_; ++col) {
      ...
      ok = simplex_info.workCost_[var] ==
           solver_lp.sense_ * solver_lp.colCost_[col];
      if (!ok) {
        printf("For col %d, simplex_info.workLower_ should be %g but is %g\n",
               col, solver_lp.colLower_[col], simplex_info.workCost_[var]);
        return ok;
      }
    }
    for (int row = 0; row < solver_lp.numRow_; ++row) {
  ...
  if (!highs_isInfinity(-simplex_info.workLower_[var])) {
    if (!highs_isInfinity(simplex_info.workUpper_[var])) {
      ...
      if (simplex_info.workLower_[var] == simplex_info.workUpper_[var]) {
        ...
        if (!ok) {
          printf("Fixed variable %d (solver_lp.numCol_ = %d) [%11g, %11g, "
                 "%11g] so nonbasic "
                 "move should be zero but is %d\n",
                 var, solver_lp.numCol_, simplex_info.workLower_[var],
                 simplex_info.workValue_[var], simplex_info.workUpper_[var],
                 basis.nonbasicMove_[var]);
          return ok;
        }
        ...
        if (!ok) {
          printf("Fixed variable %d (solver_lp.numCol_ = %d) so "
        ...
        ok = (basis.nonbasicMove_[var] == NONBASIC_MOVE_UP) ||
             (basis.nonbasicMove_[var] == NONBASIC_MOVE_DN);
        if (!ok) {
          printf("Boxed variable %d (solver_lp.numCol_ = %d) [%11g, %11g, "
                 "%11g] range %g so "
        ...
        if (basis.nonbasicMove_[var] == NONBASIC_MOVE_UP) {
          ok = simplex_info.workValue_[var] == simplex_info.workLower_[var];
          if (!ok) {
            printf("Boxed variable %d (solver_lp.numCol_ = %d) with "
          ...
          if (!ok) {
            printf("Boxed variable %d (solver_lp.numCol_ = %d) with "
                   ...
                   "value should be %g but is %g\n",
                   var, solver_lp.numCol_, simplex_info.workUpper_[var],
                   simplex_info.workValue_[var]);
            return ok;
          }
        }
    ...
    if (!highs_isInfinity(simplex_info.workUpper_[var])) {
      ...
      ok = basis.nonbasicMove_[var] == NONBASIC_MOVE_ZE;
      if (!ok) {
        printf("Free variable %d (solver_lp.numCol_ = %d) [%11g, %11g, %11g] "
               "so nonbasic "
               "move should be zero but is  %d\n",
               var, solver_lp.numCol_, simplex_info.workLower_[var],
               simplex_info.workValue_[var], simplex_info.workUpper_[var],
      ...
      if (!ok) {
        printf("Free variable %d (solver_lp.numCol_ = %d) so work value should "
               "be zero but "
               "is %g\n",
...
bool ok_to_solve(HighsModelObject &highs_model_object, int level, int phase) {
  ...
  ok = nonbasic_flag_basic_index_ok(highs_model_object, solver_lp.numCol_,
                                    solver_lp.numRow_);
  if (!ok) {
...
void compute_dual(HighsModelObject &highs_model_object) {
  ...
  for (int i = solver_lp.numCol_; i < numTot; i++) {
    simplex_info.workDual_[i] =
        simplex_info.workCost_[i] - buffer.array[i - solver_lp.numCol_];
  }
  ...
  for (int row = 0; row < solver_lp.numRow_; row++) {
    ...
    if (var < solver_lp.numCol_)
      primal_objective_function_value +=
          simplex_info.baseValue_[row] * solver_lp.colCost_[var];
  }
  for (int col = 0; col < solver_lp.numCol_; col++) {
    if (basis.nonbasicFlag_[col])
      primal_objective_function_value +=
          simplex_info.workValue_[col] * solver_lp.colCost_[col];
  }
...
void update_pivots(HighsModelObject &highs_model_object, int columnIn,
  ...
  if (columnOut < solver_lp.numCol_)
    simplex_info.num_basic_logicals -= 1;
  if (columnIn < solver_lp.numCol_)
    simplex_info.num_basic_logicals += 1;
...
void util_analyse_lp_solution(HighsModelObject &highs_model_object) {
  ...
  for (int iCol = 0; iCol < solver_lp.numCol_; iCol++) {
    ...
    double lcColDuAct =
        -(solver_lp.colCost_[iCol] * scale.cost_) / scale.col_[iCol];
    for (int en = solver_lp.Astart_[iCol]; en < solver_lp.Astart_[iCol + 1];
         en++) {
      ...
      //      Avalue_En*dual[solver_lp.numCol_+iRow]; printf("Row %2d: %11.4g -
      //      (%11.4g*%11.4g=%11.4g) = %11.4g\n", iRow, lcSclColDuAct,
      //      Avalue_En, dual[solver_lp.numCol_+iRow],
      //      Avalue_En*dual[solver_lp.numCol_+iRow], lcSum);
      lcSclColDuAct -= Avalue_En * dual[solver_lp.numCol_ + iRow];
      lcColDuAct -= unsclAvalue_En * dual[solver_lp.numCol_ + iRow] *
  ...
  for (int iCol = 0; iCol < solver_lp.numCol_; iCol++) {
    ...
    // For scaled values
    double sclColPrIfs = max(max(solver_lp.colLower_[iCol] - sclColValue,
                                 sclColValue - solver_lp.colUpper_[iCol]),
                             0.0);
    ...
    if (rpCol) {
      ...
      printf("Scl   [%11.4g, %11.4g, %11.4g] (Pr: %11.4g; Du: %11.4g; Rs: "
             "%11.4g)\n",
             solver_lp.colLower_[iCol], sclColValue, solver_lp.colUpper_[iCol],
  ...
  for (int iRow = 0; iRow < solver_lp.numRow_; iRow++) {
    ...
    if (basis.nonbasicFlag_[solver_lp.numCol_ + iRow]) {
      ...
      } else if (basis.nonbasicMove_[solver_lp.numCol_ + iRow] ==
                 NONBASIC_MOVE_UP) {
        // At upper bound
    ...
    // Assess primal infeasibility
    // For scaled values
    double sclRowPrIfs = max(max(solver_lp.rowLower_[iRow] - sclRowValue,
                                 sclRowValue - solver_lp.rowUpper_[iRow]),
                             0.0);
    ...
    if (rpRow) {
      ...
      printf("Row %3d: [Fg = %2d; Mv = %2d] Scl = %11.4g\n", iRow,
             basis.nonbasicFlag_[solver_lp.numCol_ + iRow],
             basis.nonbasicMove_[solver_lp.numCol_ + iRow], scale.row_[iRow]);
      printf("Scl   [%11.4g, %11.4g, %11.4g] (Pr: %11.4g; Du: %11.4g; Rs: "
             "%11.4g)\n",
             solver_lp.rowLower_[iRow], sclRowValue, solver_lp.rowUpper_[iRow],
