...
HighsStatus HEkkPrimal::solve() {
  ...
  const bool near_optimal =
      info.num_primal_infeasibility == 0 && info.sum_dual_infeasibility < 1;
  if (near_optimal)
  ...
  while (solve_phase) {
    ...
    if (solve_phase == kSolvePhaseUnknown) {
      ...
      // phase
      ekk_instance_.computeSimplexPrimalInfeasible();
      num_primal_infeasibility = ekk_instance_.info_.num_primal_infeasibility;
      solve_phase = num_primal_infeasibility > 0 ? kSolvePhase1 : kSolvePhase2;
...
void HEkkPrimal::rebuild() {
  ...
  // JH_factor_use
    assert(ekk_instance_.factor_.build_synthetic_tick ==
  	 ekk_instance_.simplex_nla_.build_synthetic_tick_);
    ekk_instance_.build_synthetic_tick_ = ekk_instance_.simplex_nla_.build_synthetic_tick_;
    ekk_instance_.build_synthetic_tick_ = ekk_instance_.factor_.build_synthetic_tick;
