...
HighsStatus HEkkPrimal::solve() {
  ...
  HighsOptions& options = *ekk_instance_.opt_point_;
  ...
  while (solve_phase) {
    ...
    if (solve_phase == kSolvePhase1) {
      ...
      // been reached
      //
      // solve_phase = kSolvePhaseCleanup if, after removing bound
      ...
      assert(solve_phase == kSolvePhaseOptimal || solve_phase == kSolvePhase1 ||
             solve_phase == kSolvePhase2 || solve_phase == kSolvePhaseCleanup ||
             solve_phase == kSolvePhaseUnknown ||
             solve_phase == kSolvePhaseExit || solve_phase == kSolvePhaseError);
...
void HEkkPrimal::initialiseInstance() {
  ...
  // debug option
  const bool debug =
      ekk_instance_.opt_point_->highs_debug_level > kHighsDebugLevelCheap;
  if (num_free_col) {
  ...
  hyper_chuzc_candidate_set.setup(max_num_hyper_chuzc_candidates, num_tot,
                                  ...
                                  ekk_instance_.opt_point_->log_file_stream,
                                  debug);
}
...
void HEkkPrimal::initialiseSolve() {
  ...
  primal_feasibility_tolerance =
      ekk_instance_.opt_point_->primal_feasibility_tolerance;
  dual_feasibility_tolerance =
      ekk_instance_.opt_point_->dual_feasibility_tolerance;
  objective_target = ekk_instance_.opt_point_->objective_target;
...
void HEkkPrimal::cleanup() {
  ...
  if (ekk_instance_.opt_point_->highs_debug_level > kHighsDebugLevelCheap)
...
void HEkkPrimal::iterate() {
  ...
  if (check) {
    ...
    ekk_instance_.opt_point_->highs_debug_level = kHighsDebugLevelExpensive;
  }
  ...
  if (solve_phase == kSolvePhase1) {
    phase1ChooseRow();
    if (row_out < 0) {
      highsLogDev(ekk_instance_.opt_point_->log_options, HighsLogType::kError,
...
bool HEkkPrimal::useVariableIn() {
  ...
  ekkDebugUpdatedDual(*ekk_instance_.opt_point_, updated_theta_dual,
                      computed_theta_dual);
