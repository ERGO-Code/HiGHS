...
HighsStatus HEkkPrimal::solve() {
  ...
  if (ekk_instance_.bailoutOnTimeIterations()) return ekk_instance_.returnFromSolve(HighsStatus::Warning);
  ...
  analysis = &ekk_instance_.simplex_analysis_;
  if (solvePhase == 2) {
    int it0 = ekk_instance_.iteration_count_;
  ...
  if (debugOkForSolve(ekk_instance_, solvePhase) == HighsDebugStatus::LOGICAL_ERROR)
...
void HEkkPrimal::solvePhase2() {
  ...
  for (int iCol = 0; iCol < solver_num_tot; iCol++) {
    if (highs_isInfinity(-ekk_instance_.simplex_info_.workLower_[iCol])) {
      if (highs_isInfinity(ekk_instance_.simplex_info_.workUpper_[iCol])) {
        // Free column
  ...
  computeDualObjectiveValue(ekk_instance_);
  */
}
...
void HEkkPrimal::initialise() {
  ...
  if (!no_free_columns) HighsLogMessage(ekk_instance_.options_.logfile, HighsMessageType::INFO,
