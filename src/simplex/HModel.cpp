...
void HModel::setup_transposeLP() {
  ...
  swap(solver_lp_->numRow_, solver_lp_->numCol_);
...
void HModel::setup_tightenBound() {
  ...
  // Save column bounds
  vector<double> colLower_0 = solver_lp_->colLower_;
  vector<double> colUpper_0 = solver_lp_->colUpper_;
  ...
  for (;;) {
    ...
    for (int iRow = 0; iRow < solver_lp_->numRow_; iRow++) {
      // SKIP free rows
      if (solver_lp_->rowLower_[iRow] < -big_B && solver_lp_->rowUpper_[iRow] > big_B) continue;
      ...
      for (int k = myStart; k < myEnd; ++k) {
        ...
        double upper = value > 0 ? solver_lp_->colUpper_[iCol] : -solver_lp_->colLower_[iCol];
        double lower = value > 0 ? solver_lp_->colLower_[iCol] : -solver_lp_->colUpper_[iCol];
        value = fabs(value);
        if (upper < big_B)
          xmaxU += upper * value;
        else
      ...
      if (comp_U <= solver_lp_->rowUpper_[iRow] + 1e-7 && comp_L >= solver_lp_->rowLower_[iRow] - 1e-7)
        continue;

      double row_L = solver_lp_->rowLower_[iRow];
      double row_U = solver_lp_->rowUpper_[iRow];

      // Now see if we can tighten column bounds
      for (int k = myStart; k < myEnd; ++k) {
        double value = ARvalue[k];
        int iCol = ARindex[k];
        double col_L = solver_lp_->colLower_[iCol];
        double col_U = solver_lp_->colUpper_[iCol];
        ...
        if (new_U < col_U - 1.0e-12 && new_U < big_B) {
          solver_lp_->colUpper_[iCol] = max(new_U, col_L);
          numberChanged++;
        }
        if (new_L > col_L + 1.0e-12 && new_L > -big_B) {
          solver_lp_->colLower_[iCol] = min(new_L, col_U);
          numberChanged++;
        }
      }
    }
  ...
  for (int iCol = 0; iCol < solver_lp_->numCol_; iCol++) {
    if (colUpper_0[iCol] > colLower_0[iCol] + useTolerance) {
      ...
      if (solver_lp_->colUpper_[iCol] - solver_lp_->colLower_[iCol] < useTolerance + 1.0e-8) {
        solver_lp_->colLower_[iCol] = max(colLower_0[iCol], solver_lp_->colLower_[iCol] - relax);
        solver_lp_->colUpper_[iCol] = min(colUpper_0[iCol], solver_lp_->colUpper_[iCol] + relax);
      } else {
        if (solver_lp_->colUpper_[iCol] < colUpper_0[iCol]) {
          solver_lp_->colUpper_[iCol] = min(solver_lp_->colUpper_[iCol] + relax, colUpper_0[iCol]);
        }
        if (solver_lp_->colLower_[iCol] > colLower_0[iCol]) {
          solver_lp_->colLower_[iCol] = min(solver_lp_->colLower_[iCol] - relax, colLower_0[iCol]);
        }
      }
    }
  }
...
void HModel::util_anPrDuDgn() {
  ...
  pctDgnPrAct = 100 * pctDgnPrAct / solver_lp_->numRow_;
  ...
  pctDgnDuAct = 100 * pctDgnDuAct / solver_lp_->numCol_;

  printf(
      "anPrDuDgn: model %s: ||BcPrAct|| = %g; numDgnPrAct = %d of %d "
      "(%7.2f%%); ||NonBcDuAct|| = %g; numDgnDuAct = %d of %d (%7.2f%%)\n",
      modelName.c_str(), normPrAct, numDgnPrAct, solver_lp_->numRow_, pctDgnPrAct, normDuAct,
      numDgnDuAct, solver_lp_->numCol_, pctDgnDuAct);
  printf("GrepAnPrDuDgn,%s,%g,%d,%d,%g,%d,%d\n", modelName.c_str(), normPrAct,
         numDgnPrAct, solver_lp_->numRow_, normDuAct, numDgnDuAct, solver_lp_->numCol_);
}
#endif
