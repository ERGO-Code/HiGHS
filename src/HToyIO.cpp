...
{
  int *integerColumn;
  int RtCd = readToy_MIP_cpp(filename, m_p, n_p, maxmin, offset,
                             A,
                             b, c, lb, ub,
                             &integerColumn);
  return RtCd;
  ...
  *integerColumn = (int *)malloc(sizeof(int) * n);
  ...
  while (j <= n)
  {
    ...
    if (j < n)
    {
      (*c)[j] = rl_v;
      if (rp_rd > 0)
        printf("Read %s giving %g for (*c)[%d]\n", term, rl_v, j);
      j++;
      if (buff[end] == '\0')
      {
        printf("Error reading file: not enough numbers on row of objective.\n");
    ...
    else
    {
      *offset = rl_v;
      if (rp_rd > 0)
        printf("Read %s giving %g for offset\n", term, rl_v);
      j++;
    }
  }
  ...
  while (i < m)
  {
    ...
    (*b)[i] = rl_v;
    if (rp_rd > 0)
      printf("Read %s giving %g for (*b)[%d]\n", term, rl_v, i);
    i++;
  }
  ...
  fgets_rt = fgets(buff, buff_ln, file_p);
  if (rp_rd > 0)
    printf("LB: fgets return value = %s\n", fgets_rt);
  if (fgets_rt == NULL)
  {
    fclose(file_p);
    return 0;
  ...
  while (j < n)
  {
    ...
    (*lb)[j] = rl_v;
    if (rp_rd > 0)
      printf("Read %s giving %g for (*lb)[%d]\n", term, rl_v, j);
    j++;
    if (j < n && buff[end] == '\0')
    {
  ...
  fgets_rt = fgets(buff, buff_ln, file_p);
  if (rp_rd > 0)
    printf("UB: fgets return value = %s\n", fgets_rt);
  if (fgets_rt == NULL)
  {
    fclose(file_p);
    return 0;
  ...
  while (j < n)
  {
    ...
    (*ub)[j] = rl_v;
    if (rp_rd > 0)
      printf("Read %s giving %g for (*ub)[%d]\n", term, rl_v, j);
    j++;
    if (j < n && buff[end] == '\0')
    {
  ...
  fgets_rt = fgets(buff, buff_ln, file_p);
  if (rp_rd > 0)
    printf("IC: fgets return value = %s\n", fgets_rt);
  if (fgets_rt == NULL)
  {
    fclose(file_p);
    return 0;
