...
void HDual::iterateAn()
{
  ...
  if (EdWt_Mode == EdWt_Mode_DSE)
  {
    ...
    if (CostlyDseIt)
    {
      ...
      if (alw_DSE2Dvx_sw && (AnIterNumCostlyDseIt > lcNumIter * AnIterFracNumCostlyDseItbfSw) && (lcNumIter > AnIterFracNumTot_ItBfSw * numTot))
      {
  ...
  if (iterLg)
  {
    ...
    if (EdWt_Mode == EdWt_Mode_DSE)
    {
      ...
      if (lc_NumCostlyDseIt > 0)
        printf("; LcNum =%3d", lc_NumCostlyDseIt);
    }
    printf("\n");
  }
...
{
  if (model->intOption[INTOPT_PRINT_FLAG] != 4)
    return;
  int numIter = model->numberIteration;
  bool header = numIter % 10 == 1;
  if (header)
    iterateRpFull(header);
  iterateRpFull(false);
}
...
{
  if (model->intOption[INTOPT_PRINT_FLAG] != 1 && model->intOption[INTOPT_PRINT_FLAG] != 4)
    return;
...
void HDual::chooseColumn(HVector *row_ep)
{
  ...
  bool useUltraPrice = alw_price_ultra && row_apDensity * numCol * 10 < row_ap.ilP2 && row_apDensity < 1e-3;
#ifdef HiGHSDEV
...
void HDual::updateFtranBFRT()
{
  ...
  if (time_updateFtranBFRT)
    model->timer.recordStart(HTICK_FTRAN_BFRT);
  ...
  if (time_updateFtranBFRT)
    model->timer.recordFinish(HTICK_FTRAN_BFRT);
}
...
void HDual::iterateOpRecAf(int opTy, HVector &vector)
{
  ...
  if (rsDsty <= hyperRESULT)
    AnIter->AnIterOpNumHyperRs++;
...
void HDual::iterateRpAn()
{
  ...
  if (NumInvert > 0)
  {
    ...
    if (lcNumInvert > 0)
      printf("%7d (%3d%%) Invert operations due to primal infeasible in primal simplex\n", lcNumInvert, (100 * lcNumInvert) / NumInvert);
  }
