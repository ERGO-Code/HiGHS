...
HighsStatus HighsMipSolver::runBnb() {
  HighsStatus return_status = HighsStatus::OK;
  HighsStatus call_status;
  HighsPrintMessage(options_mip.output, options_mip.message_level, ML_ALWAYS,
  ...
  Tree tree(*(root.get()));
  tree.branch(options_mip.output, options_mip.message_level, *(root.get()));
  ...
  while (!tree.empty()) {
    ...
    tree.branch(options_mip.output, options_mip.message_level, node);
  }
  ...
  if (tree.getBestSolution().size() > 0) {
    ...
    HighsPrintMessage(options_mip.output, options_mip.message_level, ML_MINIMAL,
		      message.str().c_str());
  } else {
