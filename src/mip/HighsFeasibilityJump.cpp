...
void HighsMipSolverData::feasibilityJump() {
  ...
  highsLogUser(
      log_options, HighsLogType::kInfo,
      "Feasibility Jump code uses 'int' so isn't currently compatible with a 64-bit HighsInt. Skipping Feasibility Jump\n",
      lower_bound);
  return;
#else
    ...
    for (int i = 0; i < model->num_row_; ++i) {
        ...
        if (hasFiniteLower) {
            solver.addConstraint(external_feasibilityjump::RowType::Gte,
                                model->row_lower_[i], row_num_nz,
                                row_index_buffer, row_value_buffer, 1);
        }
        if (hasFiniteUpper) {
            solver.addConstraint(external_feasibilityjump::RowType::Lte,
                                model->row_upper_[i], row_num_nz,
                                row_index_buffer, row_value_buffer, 1);
        }
        }
    }
