...
HighsSearchWorker::HighsSearchWorker(HighsMipWorker& mipworker, HighsPseudocost&
pseudocost)
    : mipworker(mipworker),
...
// HighsSearchWorker::HighsSearch(const HighsMipSolver& mipsolver,
...
double HighsSearchWorker::checkSol(const std::vector<double>& sol,
...
bool HighsSearchWorker::orbitsValidInChildNode(
...
double HighsSearchWorker::getCutoffBound() const {
  return std::min(mipsolver.mipdata_->upper_limit, upper_limit);
}

void HighsSearchWorker::setRINSNeighbourhood(const std::vector<double>& basesol,
...
void HighsSearchWorker::setRENSNeighbourhood(const std::vector<double>& lpsol) {
  for (HighsInt i = 0; i != mipsolver.numCol(); ++i) {
    if (mipsolver.variableType(i) != HighsVarType::kInteger) continue;
    if (localdom.col_lower_[i] == localdom.col_upper_[i]) continue;
...
void HighsSearchWorker::createNewNode() {
  nodestack.emplace_back();
  nodestack.back().domgchgStackPos = localdom.getDomainChangeStack().size();
}

void HighsSearchWorker::cutoffNode() { nodestack.back().opensubtrees = 0; }

void HighsSearchWorker::setMinReliable(HighsInt minreliable) {
  pseudocost.setMinReliable(minreliable);
}

void HighsSearchWorker::branchDownwards(HighsInt col, double newub,
                                  double branchpoint) {
...
void HighsSearchWorker::branchUpwards(HighsInt col, double newlb,
                                double branchpoint) {
  NodeData& currnode = nodestack.back();
...
void HighsSearchWorker::addBoundExceedingConflict() {
...
void HighsSearchWorker::addInfeasibleConflict() {
  double rhs;
...
HighsInt HighsSearchWorker::selectBranchingCandidate(int64_t maxSbIters,
                                               double& downNodeLb,
                                               double& upNodeLb) {
  ...
  for (HighsInt k = 0; k != numfrac; ++k) {
    ...
    if (!lower_ok)
      highsLogUser(mipsolver.options_mip_->log_options, HighsLogType::kError,
                   "HighsSearchWorker::selectBranchingCandidate Error fracval = %g "
                   "<= %g = %g + %g = "
...
const HighsSearchWorker::NodeData* HighsSearchWorker::getParentNodeData() const {
  if (nodestack.size() <= 1) return nullptr;

  return &nodestack[nodestack.size() - 2];
}

void HighsSearchWorker::currentNodeToQueue(HighsNodeQueue& nodequeue) {
...
void HighsSearchWorker::openNodesToQueue(HighsNodeQueue& nodequeue) {
...
void HighsSearchWorker::flushStatistics() {
...
int64_t HighsSearchWorker::getHeuristicLpIterations() const {
  return heurlpiterations + mipsolver.mipdata_->heuristic_lp_iterations;
}

int64_t HighsSearchWorker::getTotalLpIterations() const {
  return lpiterations + mipsolver.mipdata_->total_lp_iterations;
}

int64_t HighsSearchWorker::getLocalLpIterations() const { return lpiterations; }

int64_t HighsSearchWorker::getLocalNodes() const { return nnodes; }

int64_t HighsSearchWorker::getStrongBranchingLpIterations() const {
  return sblpiterations + mipsolver.mipdata_->sb_lp_iterations;
}

void HighsSearchWorker::resetLocalDomain() {
...
void HighsSearchWorker::installNode(HighsNodeQueue::OpenNode&& node) {
...
HighsSearchWorker::NodeResult HighsSearchWorker::branch() {
...
bool HighsSearchWorker::backtrack(bool recoverBasis) {
...
bool HighsSearchWorker::backtrackPlunge(HighsNodeQueue& nodequeue) {
...
bool HighsSearchWorker::backtrackUntilDepth(HighsInt targetDepth) {
  if (nodestack.empty()) return false;
  assert(!nodestack.empty());
...
void HighsSearchWorker::solveDepthFirst(int64_t maxbacktracks) {
  do {
