...
void HCrash::crash(HModel *ptr_model, int Crash_Mode)
{
  ...
  if (Crash_Mode == Crash_Mode_Bs || Crash_Mode == Crash_Mode_TsSing) {
...
void HCrash::bixby(HModel *ptr_model, int Crash_Mode)
{
  ...
  for (int r_n = 0; r_n < numRow; r_n++)
  {
    ...
    if (bixby_vr_in_r[r_n] == no_ix)
      continue;
    if (bixby_vr_in_r[r_n] == numCol + r_n)
      continue;
    int cz_r_n = r_n;
...
bool HCrash::bixby_iz_da(HModel *ptr_model)
{
  ...
  for (int c_n = 0; c_n < numCol; c_n++)
  {
    if (crsh_c_ty[c_n] != crsh_vr_ty_fr)
      continue;
    double sense_col_cost = objSense * colCost[c_n];
    if (bixby_no_nz_c_co && sense_col_cost != 0.0)
      continue;
    n_en += 1;
  ...
  for (int c_n = 0; c_n < numCol; c_n++)
  {
    if (crsh_c_ty[c_n] != crsh_vr_ty_1_sd)
      continue;
    double sense_col_cost = objSense * colCost[c_n];
    if (bixby_no_nz_c_co && sense_col_cost != 0.0)
      continue;
  ...
  for (int c_n = 0; c_n < numCol; c_n++)
  {
    if (crsh_c_ty[c_n] != crsh_vr_ty_2_sd)
      continue;
    double sense_col_cost = objSense * colCost[c_n];
    if (bixby_no_nz_c_co && sense_col_cost != 0.0)
      continue;
  ...
  for (int c_n = 0; c_n < numCol; c_n++)
  {
    if (crsh_c_ty[c_n] != crsh_vr_ty_fx)
      continue;
    double sense_col_cost = objSense * colCost[c_n];
    if (bixby_no_nz_c_co && sense_col_cost != 0.0)
      continue;
...
void HCrash::crsh_rp_r_c_st(int mode, int Crash_Mode)
{
  ...
  if (mode == 2)
    assert(ck_su_n_bc_vr == numRow);
  if (mode == 2)
    assert(ck_su_n_nonbc_vr == numCol);
#endif
  if (mode <= 1)
  {
    for (int vr_ty = crsh_f_vr_ty; vr_ty < crsh_l_vr_ty + 1; vr_ty++)
...
void HCrash::crsh_ck_an_impl_bd()
{
  ...
  int numTighter = numTighterPrimalColLower + numTighterPrimalColUpper + numTighterDualColLower + numTighterDualColUpper + numTighterPrimalRowLower + numTighterPrimalRowUpper + numTighterDualRowLower + numTighterDualRowUpper;
  if (numTighter > 0)
  {
    printf(
  ...
  int numSlacker = numSlackerPrimalColLower + numSlackerPrimalColUpper + numSlackerDualColLower + numSlackerDualColUpper + numSlackerPrimalRowLower + numSlackerPrimalRowUpper + numSlackerDualRowLower + numSlackerDualRowUpper;
  if (numSlacker > 0)
  {
    printf(
...
void HCrash::ltssf_cz_c(HModel *ptr_model)
{
       ...
       el_n++)
  {
    ...
    int c_pri_fn_v = crsh_fn_cf_pri_v * crsh_c_ty_pri_v[crsh_c_ty[c_n]] - crsh_fn_cf_k * crsh_c_k[c_n];
       ...
       r_el_n++)
  {
    ...
    for (int el_n = Astart[c_n]; el_n < Astart[c_n + 1]; el_n++)
    {
      ...
      if (TwoD_hdr)
      {
        ...
        if ((crsh_r_pri_k_hdr[hdr_ix] == no_lk) && (crsh_r_pri_mn_r_k[pri_v] == r_k))
        {
