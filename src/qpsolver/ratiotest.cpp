...
RatiotestResult ratiotest_textbook(Runtime& rt, const Vector& p,
                                   const Vector& rowmove, Instance& instance, const double alphastart) {
  ...
  for (HighsInt j = 0; j < p.num_nz; j++) {
    ...
    double alpha_i =
        step(rt.primal.value[i], p.value[i], instance.var_lo[i], instance.var_up[i], rt.settings.ratiotest_t);
  ...
  for (HighsInt j = 0; j < rowmove.num_nz; j++) {
    ...
    double alpha_i = step(rt.rowactivity.value[i], rowmove.value[i], instance.con_lo[i],
...
RatiotestResult ratiotest_twopass(Runtime& runtime, const Vector& p,
  ...
  for (HighsInt i = 0; i < runtime.instance.num_con; i++) {
    double step_i = step(runtime.rowactivity.value[i], rowmove.value[i], runtime.instance.con_lo[i],
                         runtime.instance.con_up[i], runtime.settings.ratiotest_t);
  ...
  for (HighsInt i = 0; i < runtime.instance.num_var; i++) {
    double step_i =
        step(runtime.primal.value[i], p.value[i], runtime.instance.var_lo[i], runtime.instance.var_up[i], runtime.settings.ratiotest_t);
...
RatiotestResult ratiotest(Runtime& runtime, const Vector& p, const Vector& rowmove, double alphastart) {
  switch (runtime.settings.ratiotest) {
    case RatiotestStrategy::Textbook:
      return ratiotest_textbook(runtime, p, rowmove, runtime.instance, alphastart);
    ...
    for (double& bound : relaxed_instance.var_up) {
      ...
      return ratiotest_twopass(runtime, p, rowmove, relaxed_instance, alphastart);
