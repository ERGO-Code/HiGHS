...
void HMatrix::price_by_row_ultra0(HVector &row_ap, HVector &row_ep, int *fm_i_) const
{
  ...
  for (int i = fm_i; i < ep_count; i++)
  {
    ...
    for (int k = ARstart[iRow]; k < AR_Nend[iRow]; k++)
    {
      ...
      if (rpOps)
      {
        printf("Entry %5d: ix %4d; v %11.4g; ixEn %4d; ap_c = %6d; PV2 = %11.4g",
      ...
      if (rpOps)
      {
        printf(" ckPMZ=%6d; v0 = %11.4g; v1 = %11.4g",
               ckPackMapZ, value0, value1);
        fflush(stdout);
      }
      if (newValue)
      {
    ...
    for (int en = 0; en < row_ap.count; en++)
    {
      ...
      if (ap_pWd >= row_ap.p1SparseDaStr)
      {
        ...
        if (pointer_er)
        {
          ...
          if (pointer_er)
            printf(" - ERROR: pointer is %d", pointer);
          printf("\n");
        }
      }
      ap_array[index] = value;
...
bool HMatrix::price_er_ck_core(HVector &row_ap, HVector &row_ep) const
{
  ...
  for (int index = 0; index < numCol; index++)
  {
    ...
    if ((fabs(PriceV) > HSOL_CONST_TINY && fabs(lcPriceV) <= HSOL_CONST_TINY) || (fabs(lcPriceV) > HSOL_CONST_TINY && fabs(PriceV) <= HSOL_CONST_TINY))
    {
  ...
  bool price_er = row_apCountEr || priceEr1 > priceErTl || priceEr2 > priceErTl || row_apNormCkEr;
  if (price_er)
  {
    printf("Price error");
    if (priceEr1 > priceErTl)
      printf(": ||row_apDl|| = %11.4g", priceEr1);
    if (priceEr2 > priceErTl)
      printf(": ||row_apNZ|| = %11.4g", priceEr2);
    if (row_apNormCkEr)
      printf(": ||IxCk|| = %11.4g ||row_ap|| = %11.4g, Tl=%11.4g", row_apNormCk, row_apNorm, row_apNormCkTl * row_apNorm);
    if (row_apCountEr)
