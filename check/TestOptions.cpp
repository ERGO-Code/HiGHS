...
TEST_CASE("internal-options", "[highs_options]") {
  ...
  return_status = setOptionValue(options.logfile, "mps_parser_type_free", options.records, setting_string);
  REQUIRE(return_status == OptionStatus::ILLEGAL_VALUE);

  return_status = setOptionValue(options.logfile, "mps_parser_type_free", options.records, "fixed");
  REQUIRE(return_status == OptionStatus::ILLEGAL_VALUE);

  return_status = setOptionValue(options.logfile, "mps_parser_type_free", options.records, "False");
  REQUIRE(return_status == OptionStatus::OK);

  return_status = setOptionValue(options.logfile, "mps_parser_type_free", options.records, "F");
  REQUIRE(return_status == OptionStatus::OK);

  bool mps_parser_type_free = false;
  return_status = setOptionValue(options.logfile, "mps_parser_type_free", options.records, mps_parser_type_free);
  REQUIRE(return_status == OptionStatus::OK);

  return_status = setOptionValue(options.logfile, "mps_parser_type", options.records, true);
  ...
  return_status = setOptionValue(options.logfile, "large_matrix_value", options.records, -1);
  REQUIRE(return_status == OptionStatus::ILLEGAL_VALUE);

  return_status = setOptionValue(options.logfile, "large_matrix_value", options.records, "1");
  REQUIRE(return_status == OptionStatus::OK);

  return_status = setOptionValue(options.logfile, "small_matrix_value", options.records, -1);
  REQUIRE(return_status == OptionStatus::ILLEGAL_VALUE);

  return_status = setOptionValue(options.logfile, "small_matrix_value", options.records, "1e-6");
  REQUIRE(return_status == OptionStatus::OK);

  double small_matrix_value = 1e-7;
  return_status = setOptionValue(options.logfile, "small_matrix_value", options.records, small_matrix_value);
  REQUIRE(return_status == OptionStatus::OK);

  // Check setting string options

  return_status = setOptionValue(options.logfile, presolve_string, options.records, "ml.mps");
  REQUIRE(return_status == OptionStatus::ILLEGAL_VALUE);


  std::string model_file = "ml.mps";
  return_status = setOptionValue(options.logfile, presolve_string, options.records, model_file);
  REQUIRE(return_status == OptionStatus::ILLEGAL_VALUE);

  return_status = setOptionValue(options.logfile, presolve_string, options.records, "off");
  REQUIRE(return_status == OptionStatus::OK);

  std::string presolve = "choose";
  return_status = setOptionValue(options.logfile, presolve_string, options.records, presolve);
  REQUIRE(return_status == OptionStatus::OK);

  return_status = setOptionValue(options.logfile, model_file_string, options.records, model_file);
  REQUIRE(return_status == OptionStatus::OK);
  ...
  return_status = getOptionValue(options.logfile,
				 "mps_parser_type_free", options.records, get_mps_parser_type_free);
  REQUIRE(return_status == OptionStatus::OK);
  ...
  return_status = getOptionValue(options.logfile,
				 "small_matrix_value", options.records, get_small_matrix_value);
  REQUIRE(return_status == OptionStatus::OK);
  REQUIRE(get_small_matrix_value == small_matrix_value);
  
  std::string get_model_file;
  return_status = getOptionValue(options.logfile,
				 "model_file", options.records, get_model_file);
  REQUIRE(return_status == OptionStatus::OK);
...
TEST_CASE("highs-options", "[highs_options]") {
  ...
  // Check setting boolean options
  std::string setting_string = "fixed";
  return_status = highs.setHighsOptionValue("mps_parser_type_free", setting_string);
  ...
  bool mps_parser_type_free = false;
  return_status = highs.setHighsOptionValue("mps_parser_type_free", mps_parser_type_free);
  REQUIRE(return_status == HighsStatus::OK);
  ...
  double small_matrix_value = 1e-7;
  return_status = highs.setHighsOptionValue("small_matrix_value", small_matrix_value);
  REQUIRE(return_status == HighsStatus::OK);
  ...
  bool get_mps_parser_type_free;
  return_status = highs.getHighsOptionValue("mps_parser_type_free", get_mps_parser_type_free);
  REQUIRE(return_status == HighsStatus::OK);
  REQUIRE(get_mps_parser_type_free == false);
