...
bool GetBasisSolvesSolutionNzOk(int numRow, double* pass_solution_vector,
  ...
  for (int ix = 0; ix < *solution_num_nz; ix++) {
    ...
    if (!solution_vector[row]) {
      if (dev_run) printf("SolutionNzOk: Indexed entry solution_vector[%2d] = %11.4g\n", row,
			  solution_vector[row]);
  ...
  for (int row = 0; row < numRow; row++) {
    if (solution_vector[row]) {
      if (dev_run) printf("SolutionNzOk: Non-indexed entry solution_vector[%2d] = %11.4g\n",
...
TEST_CASE("Basis-solves", "[highs_basis_solves]") {
  ...
  if (dev_run) printf("Test 1: max_residual_norm = %11.4g (Basic column)\n",
		      max_residual_norm);
  ...
  if (dev_run) printf("Test 2: max_residual_norm = %11.4g (getBasisInverseRow)\n",
		      max_residual_norm);
  ...
  if (dev_run) printf("Test 3: max_residual_norm = %11.4g (getBasisInverseCol)\n",
		      max_residual_norm);
  ...
  if (dev_run) printf("Test 4: max_residual_norm = %11.4g (getBasisSolve)\n",
		      max_residual_norm);
  ...
  if (dev_run) printf("Test 5: max_residual_norm = %11.4g (getBasisTransposeSolve)\n",
		      max_residual_norm);
  ...
  if (dev_run) printf("Test 6: max_residual_norm = %11.4g (getReducedRow)\n",
		      max_residual_norm);
  ...
  if (dev_run) printf("Test 7: max_residual_norm = %11.4g (getReducedColumn)\n",
		      max_residual_norm);
}
