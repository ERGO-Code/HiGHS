...
TEST_CASE("HighsHessian", "[highs_hessian]") {
  ...
  const bool square_internal_hessian = true;
  ...
  REQUIRE(extractTriangularHessian(options, square_hessian) == HighsStatus::kWarning);
  if (dev_run) {
  ...
  REQUIRE(extractTriangularHessian(options, triangular_hessian) == HighsStatus::kOk);
  if (dev_run) {
  ...
  negative_diagonal_hessian.q_value_[0] = -negative_diagonal_hessian.q_value_[0];
  negative_diagonal_hessian.q_value_[4] = -negative_diagonal_hessian.q_value_[4];
  negative_diagonal_hessian.q_value_[6] = -negative_diagonal_hessian.q_value_[6];
  negative_diagonal_hessian.q_value_[8] = -negative_diagonal_hessian.q_value_[8];
  negative_diagonal_hessian.q_value_[9] = -negative_diagonal_hessian.q_value_[9];
  REQUIRE(assessHessian(negative_diagonal_hessian, options, ObjSense::kMaximize) == HighsStatus::kOk);


  // Square Hessian with only triangular entries - doubled strictly triangular entries.
