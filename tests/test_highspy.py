...
class TestHighsPy(unittest.TestCase):
    def assertEqualExpr(self, expr, vars, vals, constant=None, bounds=None):
        self.assertEqual(list(map(int, expr.vars)), list(map(int, vars)), 'variable index')
        self.assertEqual(expr.vals, vals, 'variable values')
        self.assertEqual(expr.constant, constant, 'constant')
        self.assertEqual(expr.bounds, bounds, 'bounds')
...
#
    ...
    def test_numpy_slice(self):
        ...
        self.assertEqual(h.getLp().a_matrix_.format_, highspy.highs.MatrixFormat.kRowwise)
    ...
    def test_add_multiple_constraints(self):
        ...
        added_constraints = h.addConstrs([
            x + y <= 5, 
            2*x + 3*y >= 10, 
            x - z == 2])
