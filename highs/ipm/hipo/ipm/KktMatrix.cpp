...
Int KktMatrix::buildNEvalues(const std::vector<double>& scaling) {
  ...
  // computing values in parallel is always beneficial
  highs::parallel::for_each(
      0, m,
      [&](Int start, Int end) {
      ...
      // choose grainsize so that the number of tasks in the for_each loop that
      // execute the function is roughly kParallelNEValuesTasks
