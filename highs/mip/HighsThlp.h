...
#ifndef HIGHS_THLP_H_
#define HIGHS_THLP_H_
...
struct THLPData {
  ...
  bool parseCABFile(const std::string& filename, int p_ = 3,
                    double alpha_ = 0.8);
  bool parseAPFile(const std::string& filename, int p_ = 3,
                    double alpha_ = 0.8); 
  ...
  bool checkNullCols(const HighsInt num_col,
  ...
  bool legalSolution(std::vector<double>& solution) const;
  bool feasibleSolution(const HighsInt num_col,
...
class RKOAlgorithm {
 public:
  ...
  virtual bool run(RkoSolution& sol,
                   ...
                   const RKOConfig& config, bool verbose = true) = 0;
...
class SAAlgorithm : public RKOAlgorithm {
 public:
  ...
  bool run(RkoSolution& sol, std::function<double(RkoSolution&)> evaluator,
           const RKOConfig& config, bool verbose = true);
};
...
class RKOOptimizer {
 ...
 private:
  ...
  bool runSingleAlgorithm(RKOAlgorithm& algorithm, RkoSolution& sol,
                          std::function<double(RkoSolution&)> evaluator);
