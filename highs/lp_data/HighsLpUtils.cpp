...
HighsStatus withoutIndicatorConstraints(
  ...
  for (HighsInt indicator_n = 0; indicator_n < num_indicator; indicator_n++) {
    ...
    double M_lower =
        lower > -kHighsInf ? std::max(0.0, lower - min_activity) : 0;
    double M_upper =
        upper < kHighsInf ? std::max(0.0, max_activity - upper) : 0;
    const bool both_excessive =
        M_lower > kMaxIndicatorBigM && M_upper > kMaxIndicatorBigM;
    if (M_lower > kMaxIndicatorBigM || M_upper > kMaxIndicatorBigM) {
      std::stringstream ss;
      ss.str(std::string());
      ss << highsFormatToString("Indicator constraint %d has",
                                int(indicator_n));
      if (M_lower > kMaxIndicatorBigM)
        ss << highsFormatToString(
            " lower activity of %g => lower big-M value of %g", min_activity,
            M_lower);
      if (M_upper > kMaxIndicatorBigM)
        ss << highsFormatToString(
            "%s upper activity of %g => upper big-M value of %g",
            both_excessive ? ";" : "", max_activity, M_upper);
      ss << highsFormatToString(
          ": big-M value%s exceed%s %g so reformulation not possible",
          both_excessive ? "s" : "", both_excessive ? "" : "s",
          kMaxIndicatorBigM);
