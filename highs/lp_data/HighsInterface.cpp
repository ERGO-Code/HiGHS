...
HighsStatus Highs::elasticityFilter(const double global_lower_penalty,
  ...
  // Work with a copy of the IIS as it will be cleared when solving the e-LP
  this->iis_.clear();
  HighsIis iis = this->iis_;
  ...
  if (kIisDevReport)
    printf(
        "\nElasticity filter after %d passes enforces bounds on %d cols and "
        "%d "
        "rows\n",
        int(loop_k), int(num_enforced_col_ecol), int(num_enforced_row_ecol));
...
void Highs::reportSubSolverCallTime() const {
  ...
  for (HighsInt Ix = 1; Ix < kSubSolverCount; Ix++) {
    ...
    for (HighsInt thread_ix = 0; thread_ix < HighsInt(used_thread.size()); thread_ix++) {
      HighsInt thread_num = used_thread[thread_ix];
      HighsInt num_call = thread_num == 0 ?
	this->sub_solver_call_time_.num_call[Ix] :
